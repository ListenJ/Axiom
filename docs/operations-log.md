...
- **Commit**：`<pending>` `fix(cron): 未捕获rejection兜底与DB锁重试，Bun.cron回调包catch，进程不再因SQLITE_BUSY退出`（含 `scheduler.ts` + `migrate.ts` + `scheduler-crash.test.ts` + `operations-log.md`，`internal211`）
