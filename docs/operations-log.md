...
- **Commit**：fix(kg): 审计 S2 M4 空文档守卫+M5 节点身份哈希+L1 created_at 溯源+L3 DDL 单源（TDD，含 `src/kg/enhanced.ts` + `src/kg/schema.ts` + `src/crawl/processor/kg-writer.ts` + `src/mcp/server/kg-tools.ts` + `src/knowledge/pipeline.ts` + `tests/kg-content-hash.test.ts` + `tests/kg-empty-doc.test.ts` + `docs/operations-log.md`） — OPSLOG-S2-KG-HASH-PENDING
