...
- **Commit**：fix(kal): 审计 S3 M14 queryKG 返回原样 kg_nodes.id + L2 vaultNodeIdToPath 容量上界（TDD，含 `src/kal/knowledge-access-layer.ts` + `tests/kal-references.test.ts` + `docs/operations-log.md`） — __S3_COMMIT_HASH_ANCHOR__
