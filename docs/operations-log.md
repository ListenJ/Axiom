...
- **Commit**：`pending` `fix(sandbox): 换行/backtick/$()/转义与args前置拒绝，闭合Low缺口，二层鉴权与terminal一致`（含 `spawn-env.ts` + `process-sandbox.ts` + `sandbox-escape.test.ts` + `operations-log.md`，`internal211` pending）
