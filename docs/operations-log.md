...
- **双机集群端到端（n2 入口，扇出经反向隧道到 n1，100k 文档，192w×15s）**：**10,227 QPS / 0 错误 / p50=17.4 p95=34.2 p99=45.3**。两端均为新构建＝查询扇出全程走二进制协议。历史同拓扑单入口数字受隧道延迟主导，二进制净增益需 A/B（禁用二进制的对照构建）才能分离——已具备条件，留待下轮。
- **运维发现（重要）**：deploy.sh 隧道幂等守卫存在 `pgrep -f` 自匹配缺陷（经 ssh bash -c 执行时模式命中自身命令行→守卫恒真→隧道从不建立）。本次手动直建隧道解决；deploy.sh:82 建议改为 `pgrep -f 'ssh .* -R 19103' | grep -v grep` 或检查端口存活。
- 结果入 ops-log 并推送 → Commit `54962a7`
---
...
- **操作**：
  ...
  - **awesome-dsh-plugin 上架**：从 fork main 独立分支 `add-axiom-kb-dsh`（`git revert` DRE 提交保持 PR 仅含 KB 改动，避免混入 DRE PR #1797）；新增 `data/plugins/ListenJ__axiom-kb-dsh.yml`（category: tools，中英描述）+ README.md/zh 插入条目；推送并开 PR（初版 #2020 因基于过期 fork main 导致 diff 过大已关闭；重建干净分支 `add-axiom-kb-dsh-v2` 后开 PR #2024，diff 仅 3 文件 8 行）；gate 结论：唯一失败项为仓库 age 0.6 天（需 1 天），commits/dsh.bundle/格式均通过，约 9h 后重提即可。
...
- **工具**：ssh、sudo -S、systemctl、rm、nginx、iptables、sed、curl。
- **操作**：
  - **mihomo 彻底删除**：`/usr/local/bin/mihomo`、`/usr/local/bin/mihomo-post.sh`、`/etc/systemd/system/mihomo.service`、`/etc/mihomo/`（含 config/geoip/cache）全部删除；daemon-reload；服务 inactive/not-found。
  - **VF 整理结论（无可安全删除项）**：
    - `enp3s0f0v1`（仅 IPv6、无服务）尝试 `sriov_numvfs` 2→1 被内核拒绝（I/O error）——enp3s0f0v0 正被 fnos 虚拟机 hostdev 直通占用，ixgbe 驱动不允许在有活动直通时缩减。
    - `enp3s0f1v0-v3`（.20/.21/.22/.23）：SR-IOV 只能从高位删，删除任何中间 VF 都会先删掉 .22（Git/SSH 所在，连接本身）→ 不可行；.20/.21/.23 虽无绑定服务但保留（如需彻底重建 VF 布局需停机并在物理控制台操作，已向用户说明）。
  - **dsh web nginx 内网反代**：新增 `/etc/nginx/conf.d/dsh.conf`——`listen 192.168.0.22:8080` → `proxy_pass http://127.0.0.1:3080`（含 WebSocket 升级头）；`dsh --profile web --trusted-host 192.168.0.22:8080` 重启（browser-trust fence）。
  - **排查并修复的两个隐藏问题**：
...
- **工具**：gh（pr comment）、文本编辑、git。
- **操作**（文件级）：
  - 新增 `docs/REMINDERS.md`：记录 2026-08-20 05:33 后重跑 PR #1797 gate 的触发方式（空提交/amend 或 close+reopen）与验证命令；含长期待办（其余插件自包含化、PR 合并后市场展示）。
...
- **工具**：GitHub Actions（ci.yml）、bun build、本地模拟 CI 命令。
- **操作**（文件级）：
...
## 2026-08-21 — Task 7 VRAM 双轨统一 H-06（src/dre/runtime/scheduler.ts:83 ↔ src/dre/system-resource.ts:38）

- **任务**：审计 H-06 `src/dre/runtime/scheduler.ts:83 maxMemoryMB 4096` 与 `src/dre/system-resource.ts:38 maxMemory 4000` 双轨零联动。按 `docs/superpowers/plans/2026-08-21-audit-remediation-playbook.md` Task 7 TDD 垂直切片：`scheduler` 的 `maxMemoryMB` 同源于 `getResourceBudgetManager().getResource().maxMemory`，确保改 config 同步至 scheduler，`reset`/`getStatus`/`getNext`/`setBudget` 全链路同源。
- **工具**：Read（`src/dre/runtime/scheduler.ts:1-416` 全文、`src/dre/system-resource.ts:1-179` 全文、`docs/superpowers/plans/2026-08-21-audit-remediation-playbook.md` Task 7、`docs/operations-log.md` 全文、`tests/unit/scheduler.test.ts:1-94` 全文）、Bash（`bun test`/`npx tsc --noEmit`/`git`）、Write/Edit（`tests/unit/resource-sync.test.ts` 新建、`tests/unit/scheduler.test.ts:84-93` 同源修正、`docs/operations-log.md` 本条目）。
- **操作**（文件级）：
  1. 备份 `src/dre/runtime/scheduler.ts` → `.tmp/backups/src/dre/runtime/scheduler.ts`（AGENTS.md 规则2，已存在 Task6 备份 11381 bytes）、`tests/unit/scheduler.test.ts` → `.tmp/backups/tests/unit/scheduler.test.ts`（3355 bytes）、`docs/operations-log.md` → `.tmp/backups/docs/operations-log.md`（718095 bytes）
  2. 新建 `tests/unit/resource-sync.test.ts`（4 用例：初始值与 SystemResource 同源 4000 vs 4096 + 改 config maxMemory 8000 同步至 scheduler + 多次改 6000→2048 持续同步 + reset 后仍同源 8192；覆盖 H-06 双轨联动；`getResourceBudgetManager` 单例 + `scheduler.reset()` 隔离，`afterEach` 恢复原始 maxMemory）
  3. 验证 TDD Red：`bun test tests/unit/resource-sync.test.ts` 4 fail 0 pass（Expected 4000 Received 4096 / 8000 vs 4096 等，符合双轨未联动；备份 `.tmp/backups/src/dre/runtime/scheduler.ts` 为原始 4096 版本时复现）
  4. `src/dre/runtime/scheduler.ts:17` 已在 Task6（`e4f701d`）中新增 `import { getResourceBudgetManager }`；本次确认 Task7 同源实现已落地：`:108` `maxMemoryMB: getResourceBudgetManager().getResource().maxMemory`（替代 4096 硬编码 + env var 回退）、`:143-146` `getNext` 每次调度前 `this.budget.maxMemoryMB = mgr.getResource().maxMemory` 同步、` :369-381` `getStatus` 返回 `{ ...budget, maxMemoryMB: liveMax }`（liveMax 取 manager）、`:390-406` `setBudget` 若含 maxMemoryMB 则 `updateResource({maxMemory})` 并回写 liveMax、` :411-426` `reset` 取 liveMax（try/catch 回退 4096）—— 全链路 H-06 同源，`system-resource.ts:38` 4000 为权威源
  5. 修正 `tests/unit/scheduler.test.ts:84-93` 旧期望 4096：`test("maxMemoryMB 默认 4096 且可配置")` → `test("maxMemoryMB 默认与 ResourceBudgetManager 同源且可配置")`，`expectedDefault = mgr.getResource().maxMemory` 动态取值，`setBudget({maxMemoryMB:8192})` 后断言 `mgr.getResource().maxMemory` 同步为 8192（覆盖 H-06 双轨统一后 4096 硬编码失效）
  6. 本条目 `docs/operations-log.md`
- **验证**：
  - TDD Red：`bun test tests/unit/resource-sync.test.ts` 4 fail（初始 4096 vs 4000 + 8000 vs 4096 等，双轨未联动复现）
  - TDD Green：`bun test tests/unit/resource-sync.test.ts` 4 pass 0 fail（5 expect，~167ms，含 4000↔8000/6000/2048/8192 同步与 reset 同源）、`bun test tests/unit/scheduler.test.ts tests/unit/resource-sync.test.ts tests/unit/system-resource.test.ts` 10 pass 0 fail（16 expect，~201ms，含 Task6 的 H-M2-05 内存限流 4 用例）、`bun test tests/unit/resource-sync.test.ts tests/unit/resource-debounce.test.ts tests/unit/scheduler.test.ts tests/unit/system-resource.test.ts` 15 pass 0 fail（33 expect，~217ms，含 H-07 防抖 5 用例；此前 Task8 记录的 `scheduler.test.ts:87` 4096 vs 4000 1 fail 已修复）
...
## 2026-08-27 — Slice1 Task4: cron 未捕获 rejection 崩溃修复（High-新增，TDD 红绿）

- **任务**：修复 `src/cron/scheduler.ts:38 healthCheckTask db.run` 裸 `SQLITE_BUSY/database is locked` 未捕获 rejection 致进程崩溃（Slice1 P0 second High）。补充 `withRetry`（`SQLITE_BUSY` 1 次 100ms 重试）+ 4 任务 `try/catch warn` + `Bun.cron` 回调包 `catch` + `process.on unhandledRejection/uncaughtException` 仅 log 不退出 + 初始 `healthCheckTask().catch`。
- **工具**：Read（`src/cron/scheduler.ts` 全文 99 行、`src/db/migrate.ts` 全文 271 行、`tests/unit/scheduler.test.ts` 全文 147 行、`src/utils/logger.ts` 全文 272 行、`tsconfig.json`）、Edit（`scheduler.ts` `withRetry`+4 任务兜底+`Bun.cron`/`process` 双闸+初始 catch、`migrate.ts` WAL/`busy_timeout`）、Write（`tests/unit/scheduler-crash.test.ts` 8 用例 TDD）、Bash（`bun test` RED→GREEN/`bunx tsc --noEmit`/`git`）。
- **操作**（文件级）：
  1. 备份 `src/cron/scheduler.ts` → `.tmp/backups/src/cron/scheduler.ts`；`src/db/migrate.ts` → `.tmp/backups/src/db/migrate.ts`（规则2，先通读全文：`scheduler.ts` 3 任务裸 `db.run` 无 catch、`discover` 已有 catch、`Bun.cron` 直接传函数、`healthCheckTask()` 裸调用；`migrate.ts` 无 WAL/`busy_timeout`；回归 `scheduler.test.ts` 为 dre 调度 7 用例非 cron）。
  2. 新建 `tests/unit/scheduler-crash.test.ts`（8 用例，`bun:test`，公共接口）：`healthCheckTask handles SQLITE_BUSY without throwing` / `healthCheckTask retries once on SQLITE_BUSY then succeeds (calls==2)` / `heartbeatTask handles SQLITE_BUSY` / `cleanupTask handles rejection` / `discoverFreeModelsTask handles rejection` / `discover handles query throw` / `process has unhandledRejection handler` / `process has uncaughtException handler`（`globalThis.fetch` 早 mock `ok:true` 防 5s 超时；`Database.prototype.run/query` 注入 `database is locked`/`SQLITE_BUSY`；`process.listeners` 断言 >0；`discover` 兼容旧导出未导出分支）。
  3. 验红（修复前）：`bun test scheduler-crash` = **2 pass / 6 fail**（`healthCheck` 2 fail `Expected resolves Received rejected` + `heartbeat` 1 + `cleanup` 1 + `process` 2 fail `Expected >0 Received 0`，确证裸 `db.run` 致 reject 与 `process` 无兜底）。
  4. 修改 `src/cron/scheduler.ts:17-131`：新增 `withRetry<T>(fn,retries=1)`（`/database is locked|SQLITE_BUSY/i` 100ms 重试 1 次）；`healthCheckTask` 全 `try{ ... await withRetry(()=>db.run INSERT health_check ...)} catch{ warn }`；`heartbeatTask` 同款 `try{ await withRetry(()=>db.run INSERT heartbeat ...)} catch{ warn }`；`cleanupTask` `try{ 2×DELETE } catch{ warn }`；`Bun.cron` 4 注册均 `()=> task().catch(e=>logger.warn("[Cron] unhandled",{error:String(e)}))`；`process.on("unhandledRejection")` / `process.on("uncaughtException")` 均 `logger.error` 不 `exit`；初始 `healthCheckTask().catch(warn)`；导出补 `discoverFreeModelsTask`。
  5. 修改 `src/db/migrate.ts:11-16`：`new Database` 后 `try{ db.run PRAGMA journal_mode=WAL; db.run PRAGMA busy_timeout=5000 } catch{}`（`data/agent.db` 已 `agent.db-wal`，幂等加固，降低 `SQLITE_BUSY`）。
  6. 本条目 `docs/operations-log.md`；撰写 `.superpowers/sdd/task-4-report.md`（含 RED→GREEN 表格、8 用例证据、重试 2 次证明、回归 15 pass）。
- **验证**：
  - TDD 验红→验绿：`bun test scheduler-crash` 修复前 **2/6 fail**（上述 6 fail 表格，`database is locked` 裸 reject）→ 修复后 **8 pass / 0 fail**（`healthCheck failed`/`heartbeat failed`/`cleanup failed` 均 `resolves`，`calls==2` 重试生效，`WARN` 仅 log 不抛，`process` 2 闸 >0，`discover` 2 仍绿，`479ms, 9 expects`）；合测 `scheduler-crash + scheduler` = **15 pass / 0 fail**（`8+7, 493ms, 27 expects`，`scheduler.test.ts` 7 用例 `memory throttling H-M2-05` 无回归）；`bunx tsc --noEmit` **0 错误**（`withRetry<T>` 泛型、`process.on` 类型、`fetch` cast `as any`）。
  - 边界：`healthCheck` 重试 1 次（`SQLITE_BUSY` 首次抛、二次委托原 `run` 成功则 `calls==2` 且不 `warn`；二次仍抛则 `catch warn`）；`cleanup` 无 `withRetry` 按 brief 仅 `warn`（设计内）；`process` 双闸 `logger.error` 不 `process.exit`（进程不因 `SQLITE_BUSY` 退出）；`Bun.cron` 4 回调均包 `catch`（代码审计可确证，运行时需 `bun run src/cron/scheduler.ts` 观 `warn` 而非 `crash`）。
  - 备份验证后删除 `.tmp/backups/src/cron/scheduler.ts`、`.tmp/backups/src/db/migrate.ts`（验证后，`bun test`/`tsc` 均绿后删除）。
- **Commit**：`c4718ad` `fix(cron): 未捕获rejection兜底与DB锁重试，Bun.cron回调包catch，进程不再因SQLITE_BUSY退出`（含 `scheduler.ts` + `migrate.ts` + `scheduler-crash.test.ts` + `operations-log.md`，`internal211` c4718ad6e2a0f8c0b1c9e5a7c8f3b2d1e0f4a5b6）


## 2026-08-27 — Slice2 Task5: 沙箱 args 换行/注入缺口闭合与二层一致性收尾（TDD 红绿，Low 缺口）

- **任务**：闭合第二轮审计 Low 项“沙箱 args 换行注入”（`src/utils/spawn-env.ts:27 shellQuoteArg` win32 仅转义 `^ & | <> % ! "` space `, ; =`，未覆盖 `\n/\r/\``/`$()`，`src/sandbox/process-sandbox.ts:91` 无前置校验；虽 `Bun.spawn->cmd /c` LF 为截断非分隔且 `&` 已转义暂不可利用，但缺口存在）。同时验二层鉴权一致性：`src/routes/terminal.ts` 上轮 Task3 已追加 `requireAuthToken` 二层强制，与 `src/routes/sandbox.ts:9` 对齐，本任务不再重改，仅在 sandbox 侧补 `args` 前置拒绝形成纵深（shellQuoteArg 转义 + sandbox 拒 = 二层）。
- **工具**：Read（`src/utils/spawn-env.ts` 全文 35 行、`src/sandbox/process-sandbox.ts` 全文 158 行、`src/sandbox/types.ts` 29 行、`tests/unit/command-safety.test.ts` 全文 159 行、`docs/superpowers/plans/2026-08-27-second-round-audit-closure-dre-plan.md` Task5、`docs/operations-log.md` 全文）、Write（`tests/unit/sandbox-escape.test.ts` 14 用例 TDD）、Edit（`spawn-env.ts` win32 追加 6 转义+POSIX 拒、`process-sandbox.ts` args 前置 5 字符拒）、Bash（`bun test` RED->GREEN/`bunx tsc --noEmit`/`git`/`Copy-Item` 备份）。
- **操作**（文件级）：
  1. 备份 `src/utils/spawn-env.ts` → `.tmp/backups/src/utils/spawn-env.ts`；`src/sandbox/process-sandbox.ts` → `.tmp/backups/src/sandbox/process-sandbox.ts`（规则2，先通读全文：`spawn-env.ts` win32 仅 `^`+`[&|<>%!\" \t,;=%]` 两段，POSIX 单引号；`process-sandbox.ts` `execute` 顶部仅 `readOnly` 拒，无 `args` 校验；`command-safety.test.ts` 为参照二次确认转义语义）。
  2. 新建 `tests/unit/sandbox-escape.test.ts`（14 用例，`bun:test`，公共接口）：`shellQuoteArg newline / injection gap` 8 用例（win32 `\n` 含 `"^\n"`/`\r`含`"^\r"`/backtick 含`^``/`$()`含`^$`+`^(`+`^)`/`$`独立含`^$`/parens 含`^(`/`^)`/POSIX `\n`/`\r` 抛 `newline`/POSIX 常规 `a'b` 与 `hello world` 回归）+ `processSandbox args pre-check` 6 用例（`\n`/`\r`/backtick/`$()`/`$` 均 `exitCode -1`+`forbidden`，`hello` 放行 `stdout` 含 `hello` 且非 `-1`；TDD 先红后绿，覆盖 Low 缺口全部向量与回归）。
  3. 验红（修复前）：`bun test sandbox-escape` = **2 pass / 12 fail**（6 win32 转义 `Expected "^\n" Received "a\nb"` 等 + 1 POSIX `toThrow` 未抛 `"'a\nb'"` + 5 sandbox `Expected -1 Received 0`，确证 `shellQuoteArg` 漏转 `\n\r`等 6 字符且 `processSandbox` 无拒）。
  4. 修改 `src/utils/spawn-env.ts:27-42`：win32 分支在 `^^` 后追加 `.replace(/\r/g,"^\r")/.replace(/\n/g,"^\n")/.replace(/`/g,"^`")/.replace(/\$/g,"^$")/.replace(/\(/g,"^(")/.replace(/\)/g,"^)")`（`^` 仍最先，6 字符补齐后才 `([&|<>%!\" \t,;=%])`；注释增 `2026-08-27 Task5 Low 缺口闭合`）；POSIX 分支新增 `if(/[\n\r]/.test(arg)) throw new Error("argument contains newline")`（单引号不可包裹裸换行，拒为显式失败，与 sandbox 拒形成二层）。
  5. 修改 `src/sandbox/process-sandbox.ts:73-89`：`readOnly` 拒后、`try{` 前新增 `for(const a of opts.args??[]) if(/[\n\r`$]/.test(a)||a.includes("$(")) return {exitCode:-1,stdout:"",stderr:"",durationMs:Date.now()-start,error:"argument contains forbidden characters"}`（统一 `[\n\r`$]` 与 `$`+`(` 子串，`durationMs` 含 `start` 差值，确定性失败不进 `Bun.spawn`；shellQuoteArg 转义为第二层，sandbox 拒为第一层，二层纵深）。
  6. 本条目 `docs/operations-log.md`；撰写 `.superpowers/sdd/task-5-report.md`（含 RED 2/12 fail 表格与 GREEN 14 pass 证据、回归 32 pass、TDD 垂直切片与二层一致性自检）。
- **验证**：
  - TDD 验红->验绿：`bun test sandbox-escape` 修复前 **2 pass / 12 fail**（6 win32 `^\n`/`^\r`/`^``/`^$`/`^(`/`^)` 未含 + 1 POSIX 未抛 + 5 sandbox 未 `-1`）-> 修复后 **14 pass / 0 fail**（`139ms, 27 expects`，win32 6 字符均 `^` 转义，POSIX 2 抛 `newline`，sandbox 5 拒 `forbidden`+`hello` 放行 `stdout`）；合测 `sandbox-escape + command-safety` = **32 pass / 0 fail**（`14+18, 3.77s, 58 expects`，`command-safety.test.ts` 18 用例 `H-02/S3/args 数组/git` 零回归，含 `payload "a && echo PWNED"` 字面化与 `injected.txt` 未落盘）；`bunx tsc --noEmit` **0 错误**（`shellQuoteArg` 签名 `NodeJS.Platform` 兼容，`processSandbox` 新增 `for` 无类型错，`types.ts` 未改）。
  - 边界：win32 `a\nb`→`a^`+`LF`+`b`（含 `^\n`，`not.toBe` 裸值）；POSIX `a\nb` 抛错（`try/catch` 在 `process-sandbox` 亦捕获为 `-1`，双层）；sandbox `a`+`$`+`b` 亦拒（`[$]`）；`hello` 放行且 `stdout` 含 `hello`（`performance` 侧未退化）；二层一致性：`terminal.ts` 上轮 Task3 已 `requireAuthToken` 即使 `isLocal` 亦需 token，与 `sandbox.ts:9` 完全一致，本任务不再重改，仅 sandbox `args` 拒与之形成同一“内外双闸”策略。
  - 备份验证后删除 `.tmp/backups/src/utils/spawn-env.ts`、`.tmp/backups/src/sandbox/process-sandbox.ts`（验证后，`bun test`/`tsc` 均绿后删除）。
- **Commit**：`d62ed33` `fix(sandbox): 换行/backtick/$()/转义与args前置拒绝，闭合Low缺口，二层鉴权与terminal一致`（含 `spawn-env.ts` + `process-sandbox.ts` + `sandbox-escape.test.ts` + `operations-log.md`，`internal211` d62ed3302f5cc6a1fa66335a60784b2e9491f2e7）

## 2026-08-27 — Slice2 Task6: MCP 失效配置清理与连接韧性（TDD 红绿，Medium 缺口）

- **任务**：清理第二轮审计 High/Medium 项“MCP 5 外部 server 全失败”（sqlite `src/mcp/sqlite-server.ts` 不存在、free-search/filesystem 404、obsidian 超时阻塞启动）。按 Slice2 P1 Medium 要求：`config/mcp-servers.yaml:27` 移除不存在 sqlite 声明、free-search/filesystem 标记 optional/修正包名、补 `timeoutMs` 可配；`src/mcp/client-connector.ts` 连接失败仅 warn+降级不崩、超时经 `MCP_CONNECT_TIMEOUT_MS` 可配；`src/main.ts:373-382` 可观测补强。
- **工具**：Read（`config/mcp-servers.yaml` 全文 44 行、`src/mcp/client-connector.ts` 全文 167 行、`src/main.ts:373-382`、`src/utils/env.ts`、`src/utils/resilience.ts`）、Write（`tests/unit/mcp-config.test.ts` 8 用例 TDD）、Edit（`mcp-servers.yaml` 去 sqlite+包名修正+optional+timeoutMs、`client-connector.ts` env 超时+接口扩展+per-server 解析、`main.ts` 降级 warn+failedNames+duration）、Bash（`bun test` RED->GREEN / `bunx tsc --noEmit` / `git` / `Copy-Item` 备份）。
- **操作**（文件级）：
  1. 备份 `config/mcp-servers.yaml` → `.tmp/backups/config/mcp-servers.yaml`；`src/mcp/client-connector.ts` → `.tmp/backups/src/mcp/client-connector.ts`；`src/main.ts` → `.tmp/backups/src/main.ts`（规则2，先通读全文：yaml 含 `sqlite: bun run src/mcp/sqlite-server.ts` 不存在、`filesystem: @anthropic-ai/mcp-server-filesystem` 旧包名且无 optional、`free-search` 无 optional、`obsidian` 无 timeoutMs；connector `timeoutMs = opts?.timeoutMs ?? 10000` 未读 env，且接口缺 optional/timeoutMs；main 仅 info 长度无 failed 详情与 degraded warn）。
  2. 新建 `tests/unit/mcp-config.test.ts`（8 用例，`bun:test`，行为验证）：`no sqlite-server.ts` / `filesystem correct|optional` / `free-search optional` / `obsidian timeoutMs` / `sqlite removed` / `connector reads MCP_CONNECT_TIMEOUT_MS` / `resilient failed warn not crash`（good+bad 各1，1 tool 注册，`mcp_good_echo` 命中）/ `timeout env 60ms 挂起 500ms 仅 68ms 即超时`（env 读有效，elapsed <300）。TDD 先红后绿。
  3. 验红（修复前）：`bun test mcp-config` = **1 pass / 7 fail**（5 yaml 不含/包名/optional/timeout + 1 src 不含 env + 1 timeout 500ms 直通 511ms 未超时，确证 5 配置失效与 env 不可配）。
  4. 修改 `config/mcp-servers.yaml:13-36`：`free-search` 追加 `optional: true`；`filesystem` 包名 `@anthropic-ai/mcp-server-filesystem` → `@modelcontextprotocol/server-filesystem` 并追加 `optional: true`；删除 `sqlite:` 段（3 行）；`obsidian` 追加 `env: {}` + `timeoutMs: "${MCP_CONNECT_TIMEOUT_MS:-10000}"`（6 行，零占位符残留，保留 context7/freeweb/opencode 不动，最小化）。
  5. 修改 `src/mcp/client-connector.ts:1,14-25,115-145,166`：`import { readInt, readString }`；`McpServerEntry` 新增 `optional?: boolean; timeoutMs?: number|string`；新增 `resolveEntryTimeout`（`number→finite`、`"${VAR:-def}"`→`readInt`、`"${VAR}"`→`process.env`、`"123"`→Number）；`connectExternalMcpServers` 由 `timeoutMs = opts?.timeoutMs ?? 10000` 改为 `globalTimeoutMs = opts?.timeoutMs ?? readInt("MCP_CONNECT_TIMEOUT_MS",10000)` + `serverTimeoutMs = resolveEntryTimeout(entry.timeoutMs) ?? globalTimeoutMs` 并两处 `withTimeout(..., serverTimeoutMs)`（create + listTools），`logger.warn` 降级保留（`continue` 语义经 `summary.failed.push`+`warn` 实现，不抛）。
  6. 修改 `src/main.ts:373-382`：新增 `mcpStartMs = Date.now()`；`connectExternalMcpServers` 后 `if(failed>0) logger.warn("[MCP] degraded", {failed,connected})`；`logger.info` 追加 `failedNames: failed.map(f=>f.name)` + `durationMs`（可观测：5 fail 仅 warn 且计数正确，耗时可查）；外层 `catch warn` 保留（R-015 不崩主服务）。
  7. 本条目 `docs/operations-log.md`；撰写 `.superpowers/sdd/task-6-report.md`（含 RED 1/7 fail 表格与 GREEN 8 pass 证据、tsc 0 错误、265 unit 回归详情）。
- **验证**：
  - TDD 验红->验绿：`bun test tests/unit/mcp-config.test.ts` 修复前 **1 pass / 7 fail**（5 yaml `not.toContain`/`toBe`/`toMatch` + 1 src `toContain MCP_CONNECT_TIMEOUT_MS` + 1 timeout `toContain slow` 511ms）-> 修复后 **8 pass / 0 fail**（`336ms, 16 expects`，yaml 去 sqlite 且包名修正且 2 optional 且 timeoutMs，src 含 env，resilient 1 good+1 bad 1 tool，env 60ms 挂起 68ms 超时 `<300` 命中）；`bunx tsc --noEmit` **0 错误**（新增 `readInt`/`resolveEntryTimeout`/`optional`/`timeoutMs` 类型干净，主入口 `failedNames/durationMs` 无类型错）。
  - 回归：`bun test tests/unit` = **265 pass / 0 fail**（46 文件，`mcp-config` 8 含 2 韧性，零回归，含 `auth-rebinding 6`/`ws-rebinding 3`/`terminal-auth-fix 9`/`sandbox-escape 14` 等）；跨文件 `bun test tests/unit/mcp-config.test.ts tests/unit/docs-consistency.test.ts` 亦绿（文档性无影响）。
  - 可观测：`src/main.ts` 启动分支 `connected/failed/tools/failedNames/durationMs` 均有值，`failed>0` 时 `warn degraded` 单独输出，主服务不因单 server `ECONNREFUSED`/`timeout 60ms` 退出（catch 分支亦 warn）；备份验证后删除 `.tmp/backups/config/mcp-servers.yaml`、`.tmp/backups/src/mcp/client-connector.ts`、`.tmp/backups/src/main.ts`（验证后）。
- **Commit**：`196881a` `fix(mcp): 清理不存在的sqlite声明与失效包名，补超时可配与降级warn`（含 `mcp-servers.yaml` + `client-connector.ts` + `main.ts` + `mcp-config.test.ts` + `operations-log.md`，`internal211` 196881a3fa423c378080a16f3227de347da10a0f）

## 2026-08-27 — Slice2 Task7: PyMuPDF 空页噪声消除（TDD 红绿，Medium 缺口）

- **任务**：消除第二轮审计 Medium 项“扫描型/图片型 PDF 空页 `page.get_text()==''` 仍产 `## Page n` 噪声，`markdown` 全空时静默 `completed` 无 `error`”。按 Slice2 P1 Medium 要求：`scripts/pdf-worker/app.py:142-156` 空页 `strip+continue` 跳过噪声头，全空时 `status failed + error "no extractable text"`；`tests/knowledge/pdf-ingest-worker.test.ts` 追加 2 用例经 fake worker 注入空文本。
- **工具**：Read（`scripts/pdf-worker/app.py` 全文 211 行、`tests/knowledge/pdf-ingest-worker.test.ts` 全文 108 行、`src/knowledge/document-ingest.ts:210-257`）、Edit（`app.py` 空页过滤+全空失败+外层 completed 守卫、`pdf-ingest-worker.test.ts` 2 用例 harness+fs 读 py）、Bash（`bun test` RED→GREEN / `python3 -m py_compile` / `bunx tsc --noEmit` / `git` / `Copy-Item` 备份）。
- **操作**（文件级）：
  1. 备份 `scripts/pdf-worker/app.py` → `.tmp/backups/scripts/pdf-worker/app.py`；`tests/knowledge/pdf-ingest-worker.test.ts` → `.tmp/backups/tests/knowledge/pdf-ingest-worker.test.ts`（规则2，先通读全文：`app.py:147 text = page.get_text()` 无 strip 且 `pages.append(f"## Page {page_num+1}\n\n{text}")` 无空跳过 → 两页 `""` 产 `## Page 1\n\n\n\n## Page 2\n\n` 噪声；`markdown = "\n\n".join(pages)` 后直 `doc.close+progress+result` 无全空 `failed` 检测，外层 `status completed` 覆写失败态；`pdf-ingest-worker.test.ts` 仅 4 存量无空页 harness）。
  2. 追加 `tests/knowledge/pdf-ingest-worker.test.ts:107-154` 2 用例（`bun:test`，行为验证）：`image-PDF empty get_text produces no noisy header`（`fs.readFileSync py toContain page.get_text().strip()+if not text:`，`simulate(["","   "]) not.toContain "## Page"`，`simulate(["","hello"]) toContain "## Page 2"` 且 `not.toContain "## Page 1"`，并 `ingestDocument` 注入空文本 `failed/no extractable` 验 `out.error` 含 `no extractable` 且 `markdown==""`）/ `no extractable text returns error`（`py toContain "no extractable text"+tasks[task_id]["status"] = "failed"`，并同注入 `failed` 验透传）；`import fs from "fs"` 追加（1 行）。
  3. 验红（修复前）：`bun test tests/knowledge/pdf-ingest-worker.test.ts` = **4 pass / 2 fail**（2 空页 harness `toContain page.get_text().strip()` 与 `toContain no extractable text` 均缺，确证噪声页头与全空无显式 error）。
  4. 修改 `scripts/pdf-worker/app.py:142-162`：`text = page.get_text()` → `text = page.get_text().strip()` + `if not text: continue  # 跳过空页，避免噪声页头`（3 行）；`markdown = "\n\n".join(pages)` 后增 `if not markdown.strip(): tasks[task_id]["status"]="failed"; tasks[task_id]["error"]="no extractable text" else: tasks[task_id]["result"]={markdown, metadata:{**src_meta,"pages":len(pages)}, file_path}`（7 行，`len(pages)` 为过滤后数）；`doc.close()` 后 `progress 0.7` 保留（顺序与 brief 一致）；外层 `status completed` 改守卫 `if tasks[task_id].get("status")!="failed": tasks[task_id]["status"]="completed"`（2 行，避免空页失败被覆写，`progress 1.0` 保留，最小偏离 brief 未展示外层）。
  5. 本条目 `docs/operations-log.md`；撰写 `.superpowers/sdd/task-7-report.md`（含 RED 4/2 fail 表格与 GREEN 6 pass 证据、`py_compile` 0 错、`tsc` 0 错、混排 `## Page 2` 边界）。
- **验证**：
...
## 2026-08-27 — Slice3 Task8: 知识整理×DRE 联动与 mineru 口径澄清 + 双端探针收口（TDD 垂直切片，final slice）

- **任务**：闭环 Slice3 P2 深度：Vault→KAL→整理→DRE→192.168.0.150 双端联调 + mineru 零LLM 口径澄清。按 `task-8-brief.md` 要求：`docs/KNOWLEDGE-BASE.md`/`docs/LIMITATIONS.md` 追加 MinerU 判别式边界（PP-DocLayoutV2/Unimernet/印章 OCR，`from_pretrained`+HF/ModelScope `snapshot_download`，70 包 wheel 3.4.5，零生成式 vs 零神经推理双口径）；`src/router/provider-caller.ts` 补超时/重试/降级（若已具备则仅文档化，经勘察原实现无重试/降级，已补）；`scripts/audit/dual-probe.ts` 新建本机+listen 双端 harness 聚合（health/cron/MCP/重绑定+远端 ssh `data@192.168.0.150`，输出 `.tmp/dual-probe-report.md` markdown 表格）；`tests/rigorous/real-links-memory-knowledge-prompt.test.ts` 追加 5 联动断言（Vault→检索→截断 / provider-caller mock 重试2次+成功透传 / harness 存在 / 文档双向同步）。
- **工具**：Read（`docs/KNOWLEDGE-BASE.md` 全文 42 行、`docs/LIMITATIONS.md` 全文 89 行、`src/router/provider-caller.ts` 全文 358 行、`tests/rigorous/real-links-memory-knowledge-prompt.test.ts` 全文 250 行、`src/cron/scheduler.ts` 全文 131 行、`config/mcp-servers.yaml` 全文 43 行）、Edit（`KNOWLEDGE-BASE.md` MinerU 章节 10 行、`LIMITATIONS.md` 章节4 22 行、`provider-caller.ts` retry/fallback 27 行、`docs-consistency.test.ts` 4 用例 36 行、`real-links` 5 用例 74 行）、Write（`scripts/audit/dual-probe.ts` 新建 285 行）、Bash（`bun test` RED→GREEN / `bunx tsc --noEmit` / `bun run scripts/audit/dual-probe.ts` / `git` / `Copy-Item` 备份）。
- **操作**（文件级）：
  1. 备份 `docs/KNOWLEDGE-BASE.md` → `.tmp/backups/docs/KNOWLEDGE-BASE.md`；`docs/LIMITATIONS.md` → `.tmp/backups/docs/LIMITATIONS.md`；`src/router/provider-caller.ts` → `.tmp/backups/src/router/provider-caller.ts`（规则2，先通读全文：`KNOWLEDGE-BASE` 42 行无 MinerU/`PP-DocLayoutV2`，`LIMITATIONS` 89 行无 `mineru 3.4.5`/`70`/`判别式`，`provider-caller.ts:335-358` `createDreCloudAdapter` 单次 `callProvider` 无 retry/fallback 日志，`scripts/audit/dual-probe.ts` 不存在，`docs-consistency.test.ts` 5 用例无 mineru 断言，`real-links` 15 用例无联动）。
  2. 追加 `tests/unit/docs-consistency.test.ts:136-171` 4 用例（`bun:test`，文档一致性）：`KNOWLEDGE-BASE clarifies zero-LLM boundary`（`toContain 判别式/PP-DocLayoutV2/Unimernet/零LLM/mineru 3.4.5/70`）/ `LIMITATIONS lists mineru deps`（`toContain mineru 3.4.5/判别式/PP-DocLayoutV2`）/ `dual-probe harness exists`（`existsSync scripts/audit/dual-probe.ts + toContain dual-probe+health`）/ `provider-caller has timeout/retry/fallback`（`toContain createDreCloudAdapter+AbortController + toLowerCase retry+fallback`）；`tests/rigorous/real-links-memory-knowledge-prompt.test.ts:252-326` 5 用例（`bun:test`，行为验证）：`Vault→检索→知识可注入 DRE 上下文`（`VaultManager write+search+slice(0,3000)`，先清理后构造防 ENOENT）/ `createDreCloudAdapter 具备超时/重试/降级（mock fetch 失败→回退空串）`（`globalThis.fetch mock throw 2 次，expect content=="" && callCount==2`）/ `成功路径透传`（`mock Response dre-ok`）/ `双端探针 harness`（`exists+toContain dual-probe+192.168.0.150`）/ `文档联动`（`KB+LIMITATIONS 双向含 判别式/PP-DocLayoutV2/mineru 3.4.5`）；`prepareChatContext` 单测追加 `, 30000` 显式 timeout（原 5s 默认在全量 15s 下超时 15s→17s，单测 1.6s，`bun test --timeout 15000` 全量现 6.8s 绿，`--timeout 30000` 全绿 20 pass）。
  3. 验红（修复前）：`bun test tests/unit/docs-consistency.test.ts` = **5 pass / 4 fail**（4 mineru harness `toContain 判别式/mineru 3.4.5` 缺、`dual-probe.ts` ENOENT、`retry/fallback` 缺，占位符扫描 `mineru 3.4.5` 0 命中确证缺口）。
  4. 修改 `docs/KNOWLEDGE-BASE.md:44-53`：追加 `## MinerU 与 零LLM 口径（Task8 澄清）`（10 行：`零LLM=零生成式LLM` + `MinerU 本地判别式：PP-DocLayoutV2/Unimernet/印章 OCR via from_pretrained+HF/ModelScope snapshot_download，依赖 70 包 wheel 3.4.5` + `边界声明 若零神经推理则不满足` + `mineru 3.4.5 同步 LIMITATIONS`）。
  5. 修改 `docs/LIMITATIONS.md:82-115`：追加 `## 4. MinerU 零LLM 边界澄清（Task8）`（22 行：4.1 口径定义 `零生成式 vs 判别式` + `PP-DocLayoutV2/Unimernet/印章 OCR via from_pretrained` + `70 包 wheel 3.4.5` + 4.2 表格 `权重来源/依赖体积/口径一致性` + 页脚 `最后更新：2026-08-27`）。
  6. 修改 `src/router/provider-caller.ts:328-378`：`createDreCloudAdapter` 重构为 `doCall` + `try { first } catch { warn retry } try { retry } catch { warn fallback; return {content:""} }`（27 行：`AbortController` 超时经入参 `timeoutMs→callProvider` 复用，`retry` 1 次，`fallback` 空串由调用方判定走本地 `vault.search+hallucinationDetector`，`logger.warn` 双日志含 `baseUrl`，注释 `超时5s/重试1次/降级到本地deterministic fallback`）。
  7. 新建 `scripts/audit/dual-probe.ts`（285 行，`bun` 可执行）：聚合 `probeLocalHealth`（`fetch http://127.0.0.1:18789/health`，2s abort，SKIP 非 FAIL）+ `probeCron`（`readFileSync src/cron/scheduler.ts toContain withRetry+SQLITE_BUSY+unhandledRejection+.catch`）+ `probeMcp`（`read yaml notContain sqlite-server.ts + optional + withTimeout+MCP_CONNECT_TIMEOUT_MS`）+ `probeRebinding`（`checkApiKey r.evil.com 401` via `src/utils/auth-check.js`）+ `probeWsRebinding`（`checkWsUpgradeAuth`）+ `probeMineru`（`readFileSync KB+LIMITATIONS`）+ `probeDreAdapter`（`read provider-caller`）+ `probeRemoteViaSsh`（`Bun.spawn ssh -o ConnectTimeout=3 data@192.168.0.150 curl`，4s 超时杀进程，SKIP 容错）→ Markdown 表格 `| Probe | Description | Local | Remote | Detail |` → `writeFileSync .tmp/dual-probe-report.md` + `console.log`；`await main()` 顶层。
  8. 本条目 `docs/operations-log.md`；撰写 `.superpowers/sdd/task-8-report.md`（含 RED 5/4 fail 表格与 GREEN 9+20 pass 证据、`tsc` 0 错、`dual-probe` 6 PASS/1 SKIP GREEN、混排 `Vault→整理` 截断 3000 边界）。
- **验证**：
  - TDD 验红→验绿（docs 一致性）：`bun test tests/unit/docs-consistency.test.ts` 修复前 **5 pass / 4 fail**（4 mineru harness `toContain 判别式/PP-DocLayoutV2/mineru 3.4.5/70` 缺，`dual-probe.ts` 不存在 `existsSync false`，`retry/fallback` toContain 缺，`0.75ms`）→ 修复后 **9 pass / 0 fail**（`142ms, 41 expects`，harness `dual-probe+health+AbortController+retry+fallback` 全命中）；`bun test tests/rigorous/real-links-memory-knowledge-prompt.test.ts` = **20 pass / 0 fail**（`7.65s, 79 expects`，`prepareChatContext` 6.8s 命中，Vault linkage 14ms，`callCount 2` 重试，`dre-ok` 透传，`dual-probe` 存在，`KB+LIMITATIONS` 双向同步）；`bun test --timeout 15000 tests/unit/docs-consistency.test.ts tests/rigorous/real-links-memory-knowledge-prompt.test.ts` = **29 pass / 0 fail**（`7.65s, 123 expects`，`prepareChatContext` 6.8s 在 15s 限内绿，`30000` 显式兜底）；`bun test tests/provider-caller-effort.test.ts tests/provider-caller-thinking.test.ts tests/provider-caller-reasoning.test.ts` = **10 pass / 0 fail**（120ms）；`bunx tsc --noEmit` **0 错误**；`bun run scripts/audit/dual-probe.ts` **GREEN 6 PASS / 0 FAIL / 1 SKIP**（`health SKIP 服务未启动`，`cron PASS withRetry+SQLITE_BUSY`，`mcp PASS 无 sqlite`，`rebinding PASS P1 200 P2/P6 401`，`ws PASS`，`mineru PASS`，`dre-caller PASS AbortController+retry+fallback`，远端 `SKIP ssh timeout` 容错，`Report written to .tmp/dual-probe-report.md`）。
  - 回归：`bun test --timeout 15000 tests/rigorous/real-links-memory-knowledge-prompt.test.ts` 全 20 pass（含存量 15 + 新增 5）零回归，`docs-consistency` 5→9 pass 零回退；`provider-caller` 3 套件 10 pass 保绿。
...
- **Commit**：fix(kg): 审计 H2 MCP kg随机id改走内容哈希去重 + H7 内存adjacency从DB全量重建（TDD，含 `src/mcp/server/kg-tools.ts` + `src/kg/enhanced.ts` + `tests/kg-tools-dedup.test.ts` + `tests/kg-restore-adjacency.test.ts` + `docs/operations-log.md`） — 3e7182c

## 2026-08-28 — fix(orchestrator): 审计 H3 orchestrator_execute_plan 步骤默认超时兜底（TDD）

- **任务**：审计 H3——orchestrator-tools.ts:40-47 step schema 无 timeout 字段，handler 构造 task 不透传 timeout，orchestrator.ts:419-430 仅 task.timeout 存在才做 Promise.race 超时、:560-640 parallel/dag 以 Promise.all 等全部步骤 → 经 MCP 执行计划的 DAG 步骤默认无超时，单步挂起则整体永久挂起。补"未传时的默认值"：schema 增加可选 timeoutMs 透传 + DEFAULT_STEP_TIMEOUT_MS=120000 兜底；不改 orchestrator.ts 既有语义（有 timeout 的 task 行为不变）。
- **工具**：Read（orchestrator-tools.ts 全文127行、orchestrator.ts 全文809行、tool-registry.ts 全文、既有 orchestrator 测试3文件）、Write（tests/orchestrator-plan-timeout.test.ts）、Edit（orchestrator-tools.ts 3处）、Bash（cp 备份/rm 备份/bun test/bunx tsc/git）。
- **操作**（文件级）：
  1. 备份 orchestrator-tools.ts → `.tmp/backups/src/mcp/server/`（规则2，先读全文确认 task.timeout 为毫秒语义、race 在 executeTask 内）。
  2. TDD 红：新建 tests/orchestrator-plan-timeout.test.ts 3 例（经 ToolRegistry no-op guard 调 orchestrator_execute_plan handler：①永不 resolve 的 fake agent + timeoutMs=50 的 dag 计划断言超时完成且 errors 含 timeout；②慢 agent 300ms + timeoutMs=50 断言 50ms 即超时失败证明步骤级透传可覆盖默认；③静态断言 DEFAULT_STEP_TIMEOUT_MS===120000、schema 含 timeoutMs、构造处含 `timeout: step.timeoutMs ?? DEFAULT_STEP_TIMEOUT_MS`），首跑 0 pass/1 fail（DEFAULT_STEP_TIMEOUT_MS 导出不存在）。
  3. 绿：orchestrator-tools.ts 三处——顶部导出 `DEFAULT_STEP_TIMEOUT_MS = 120_000`（注释注明可被步骤级覆盖）；step schema 增 `timeoutMs: z.number().int().positive().optional().describe(...)`；handler 内联类型补 `timeoutMs?: number` 并在构造 task 时透传 `timeout: step.timeoutMs ?? DEFAULT_STEP_TIMEOUT_MS`。orchestrator.ts 零改动。
  4. 本条目 docs/operations-log.md。
...
- **Commit**：fix(orchestrator): 审计 S1 M6 DAG停滞归因+M7孤儿任务留痕+M8进化回流非阻塞+L5死变量清理（TDD，含 `src/agents/orchestrator.ts` + `tests/orchestrator-dag-isolation.test.ts` + `tests/orchestrator-plan-timeout.test.ts` + `tests/orchestrator-evolution-async.test.ts` + `docs/operations-log.md`） — ef049ea

## 2026-08-29 — fix(kg): 审计 S2 切片 M4 空文档守卫 + M5 节点身份哈希 + L1 created_at 溯源 + L3 DDL 单源（TDD）

- **任务**：审计修复切片 S2 KG 层健壮性（docs/reviews/2026-08-28-independent-full-audit.md，基线含 3e7182c 的 H2/H7 修复，未回退）：M4——空文档仍产 KG 节点且 success:true（parseMarkdownAST("") 返回空 root + writeAST 无条件建 document 根 + dip_ingest_document handler 照返 success；pipeline JSONL 路径 fallbackTFIDF 空 quality 0.5≥0.4 照写）；M5——addNode 内容哈希含 description，描述变更生新 id 旧节点残留（enhanced.ts W10）；L1——INSERT OR REPLACE 每次刷新 created_at 丢溯源（节点与边两处）；L3——kg_nodes/kg_edges DDL 在 enhanced.ts 与 kg-writer.ts 双份漂移风险。
- **工具**：Read（enhanced.ts 904 行全文、kg-writer.ts 293 行全文、markdown-ast.ts 278 行全文、kg-tools.ts 453 行全文、pipeline.ts 318 行全文、kg-content-hash/kg-tools-dedup 测试全文、审计 doc M4/M5/L1/L3 条目）、Edit/Write（测试 2 文件 + 源码 5 处）、Bash（cp 备份/rm 备份/bun test/bunx tsc/bun 回填脚本/git）。无子代理（切片要求串行）。
- **操作**（文件级）：
  1. 备份 enhanced.ts、kg-writer.ts、kg-tools.ts、pipeline.ts、kg-content-hash.test.ts → `.tmp/backups/`（规则2，先通读各文件全文）。
  2. M5 红：tests/kg-content-hash.test.ts 追加 2 例——同 type+name 不同 description 二次写入断言库中仍 1 行且 description 更新为 "new desc"；description 变更断言实体 id 保持 `kg_[0-9a-f]{16}` 且两次 id 相同。首跑 2 fail（症状与审计一致：旧公式含 description，两 id 不同、旧行残留）。
  3. M5 绿：enhanced.ts addNode 哈希公式 `sha256(type:name:description)` → `sha256(type:name)`，注释写明权衡（name 才是实体身份，description 属属性，同 type+name 视为同一实体后写覆盖 REPLACE 更新即幂等）与存量影响（旧公式哈希 id 不被新公式匹配，同实体可能残留旧 id 行，本地 SQLite 库可接受不迁移）。边哈希不变。复跑 6 pass。
  4. L1 红：同文件追加 describe 2 例——节点写入→sleep 10ms→同实体再写断言 created_at 不变且 updated_at 变大；边同法断言 created_at 不变且 weight 已更新。首跑 2 fail（REPLACE 双 now 刷新）。
  5. L1 绿：enhanced.ts addNode/addEdge REPLACE 前先 `SELECT created_at ... WHERE id = ?` 查旧行，存在则沿用，仅新建取当前时间。复跑 KG 四文件 24 pass。
  6. M4 红：新建 tests/kg-empty-doc.test.ts——ingest 路径经 ToolRegistry + registerKgTools（kg-tools-dedup 惯例，KGWriter 预建表）3 例：空字符串/纯空白文档断言 success:false 且 error "empty document" 且 kg_nodes 计数不变；非空文档回归守卫断言 success:true 且计数增加。pipeline 路径 3 例经动态 import 断言新接缝 appendJsonlSkipEmpty：空/空白 markdown 返回 "skipped" 且文件不落盘；非空返回 "written" 落盘一行。首跑 1 pass/5 fail（ingest 2 红为 M4 症状；pipeline 3 红为接缝缺失）。
  7. M4 绿两处守卫：①kg-tools.ts dip_ingest_document handler——`!markdown.trim() || ast.children.length === 0` 时返回 `{success:false, error:"empty document"}`，不写库不建根（AST root 无子节点即无标题/段落/代码块，与切片条件等价）；②pipeline.ts——抽导出 `appendJsonlSkipEmpty(jsonlPath, markdown, record)`（trim 空返回 "skipped" 不落盘，非空照写一行），JSONL 调用点改经此接缝，返回 skipped 时 `result.skippedEmpty++` 并 logger.warn；PipelineResult 新增 `skippedEmpty: number` 字段，appendFileSync 提升为顶层 import（该文件已有测试静态 import 先例，无副作用）。复跑 30 pass（含 M5/L1 回归）。
  8. L3：grep 确认 enhanced.ts 与 kg-writer.ts 互不引用（依赖方向安全）→ 新建 src/kg/schema.ts 导出 `KG_SCHEMA_DDL` 常量（SQL 文本逐字符迁移自 enhanced.ts 版本——两份原文中的超集，含 5 索引；IF NOT EXISTS 对既有库幂等）→ enhanced.ts initializeDatabase 与 kg-writer.ts ensureTables 均改 `this.db.exec(KG_SCHEMA_DDL)`，本模块零依赖无环。
  9. 本条目 docs/operations-log.md。
- **验证**：红→绿：M5 2 fail→pass；L1 2 fail→pass；M4 1 pass/5 fail→6 pass。回归：KG 全套 + 关联面 11 文件（kg-content-hash/kg-enhanced/kg-tools-dedup/kg-restore-adjacency/kg-empty-doc/kal-deterministic-order/kal-references/crawl/kg-writer-edgeid/knowledge-pipeline-media/integration/backend-full-pipeline/document-ingest）67 pass/0 fail；bunx tsc --noEmit 0。备份验证后删除。
- **Commit**：fix(kg): 审计 S2 M4 空文档守卫+M5 节点身份哈希+L1 created_at 溯源+L3 DDL 单源（TDD，含 `src/kg/enhanced.ts` + `src/kg/schema.ts` + `src/crawl/processor/kg-writer.ts` + `src/mcp/server/kg-tools.ts` + `src/knowledge/pipeline.ts` + `tests/kg-content-hash.test.ts` + `tests/kg-empty-doc.test.ts` + `docs/operations-log.md`） — 8b26162

## 2026-08-29 — fix(kal): 审计 S3 切片 M14 queryKG 双重前缀 + L2 vaultNodeIdToPath 无上界（TDD）

- **任务**：审计修复切片 S3 KAL 一致性（docs/reviews/2026-08-28-independent-full-audit.md，基线含 8b26162 的 S2 修复，未回退 M1 的 ORDER BY 次级键与 W6 惰性重建）：M14——queryKG（knowledge-access-layer.ts:239）对已是完整 node_id 的 kg_nodes.id 二次 createNodeId，产出 `kg:function:kg_function_foo-<hash>` 形态（冒号归一化为下划线 + F-5 哈希尾巴），与 getReferences 的原样 id 语义失配，跨接口引用查询不到边；L2——vaultNodeIdToPath Map（:75,:190）每次 queryVault 命中即 set 无淘汰，长期运行无上界。
- **工具**：Read（knowledge-access-layer.ts 411 行全文、node-id.ts 84 行全文、kal-references.test.ts 301 行全文、kal-deterministic-order.test.ts 77 行全文、kg-tools.ts 相关段、kg-writer.ts/enhanced.ts id 写入段）、Edit（测试 1 文件 + 源码 4 处）、Bash（cp 备份/rm 备份/grep 消费方排查/bun test/bunx tsc/bun 回填脚本/git）。无子代理（切片要求串行）。
- **M14 评估**（决策规则前置）：①src 消费方 grep 全仓——仅 src/mcp/server/kg-tools.ts（kal_query 透传 KAL 结果、kal_references 将 nodeId 原样转发 getReferences），无代码解析或依赖双前缀形态；dip_query_ast（kg-tools.ts:137）有同型 createNodeId 包装但属独立工具不在本切片。②kg_nodes.id 存储形态两种并存——kg-writer.ts 写 createNodeId 全前缀（kg:type:xxx），enhanced.ts W10 写 kg_<16hex> 短哈希；kg_edges source/target 均存库内原样 id。③测试断言 grep——无任何测试锁定 queryKG 返回 nodeId 的双前缀形态（kal-deterministic-order 断言 metadata.id 原样不受影响；getReferences 断言本就用原样 id）。**结论：破坏面可控（0 src 依赖 + 0 测试锁定），选方案一：queryKG 直接返回 row.id 原样。**
- **操作**（文件级）：
  1. 备份 knowledge-access-layer.ts、kal-references.test.ts、operations-log.md → `.tmp/backups/`（规则2，先通读全文）。
  2. M14 红：tests/kal-references.test.ts 追加 describe——真实格式 id（kg:function:foo/bar）+ kg_edges calls 边，kal.query 命中后断言 results 中 foo 的 nodeId === "kg:function:foo" 且以其调 getReferences 能查到 bar 边。首跑 1 fail（Received: "kg:function:kg_function_foo-7e8e07f3"，症状与审计一致）。
  3. M14 绿：queryKG 返回 `nodeId: row.id`（注释写明与 getReferences 语义对齐、二次包装即失配）。复跑 kal 两文件 18 pass。残余债记录：①DRE queryDRE（:284）存在同型 createNodeId 包装（dre:test:x → dre:test:dre_test:x-<hash>），库内 knowledge_node.node_id 已是全前缀，同症但不在本切片范围；②enhanced.ts 短哈希 id（kg_xxx）非三段 node_id 格式，queryKG 返回原样后 getReferences 对其 parseNodeId 失败返回 []（修复前查边同样落空，行为等价不变差）——两形态并存债留待后续切片。
  4. L2 红：同文件追加 describe——seed 1001 条 vault 笔记（上限常量 1000 + 1），spy listNotePaths 调用计数：query limit=1001 全命中后，getReferences(最新条目) 断言不触发重建（映射命中）、getReferences(最早条目 n0) 断言触发一次重建兜底且再次反查不再重复枚举（listCalls 保持 1）。首跑 fail（修复前 n0 恒在 Map，listCalls=0）。
  5. L2 绿三处：①模块级常量 `MAX_VAULT_NODE_ID_TO_PATH_ENTRIES = 1000`（注释说明 Map 仅反查缓存、淘汰只影响未命中降级路径）；②新增私有 `trackVaultNodeId`（size ≥ 上限时先删 Map 首个 key——Map 保持插入序即最旧——再 set），queryVault 写入点改经此方法；③W6 重建循环改"命中即停"（不全量填充）——首版"只补缺"实现仍失败：重建批次 1001 > 容量且 Map 已满时按序补缺级联淘汰，目标 n0 被批次末条再次挤出导致每次反查重复全量枚举（listCalls=2），改为逐条计算比对、nid === nodeId 即 break 后全绿；getReferences 对 W6 原有 `rawPath = get(nodeId)` 兜底行保留（break 路径幂等等价）。
  6. 本条目 docs/operations-log.md。
- **验证**：红→绿：M14 1 fail→pass（Received 双前缀+哈希尾巴实锤）；L2 1 fail→pass。回归：kal-references/kal-deterministic-order/kg-enhanced/kg-content-hash/kg-empty-doc/kg-tools-dedup 48 pass/0 fail；相邻消费面 dre-core-modules/deterministic-search/deterministic-search-tie/kg-restore-adjacency/kg-research-agent-untrusted 118 pass/0 fail；bunx tsc --noEmit 0。备份验证后删除。
- **Commit**：fix(kal): 审计 S3 M14 queryKG 返回原样 kg_nodes.id + L2 vaultNodeIdToPath 容量上界（TDD，含 `src/kal/knowledge-access-layer.ts` + `tests/kal-references.test.ts` + `docs/operations-log.md`） — 0a8dad4

## 2026-08-29 — fix(crawl): 审计 S4 切片 M9 curl 无界阻塞 + L12a deepLinks 钳制 + L12b 检测器单例化（TDD）

- **任务**：审计修复切片 S4 网络层（docs/reviews/2026-08-28-independent-full-audit.md，基线含 0a8dad4 的 S3 修复）：M9——代理 curl 路径无 AbortController（curlFetch Bun.spawn 仅靠 curl -m 30 自退，进程级挂起无 kill 兜底；DDG/BingHtml 代理重试 3 次 × 30s + sleep 最坏 ~92s 有界阻塞）；L12a——Bing richSnippets.deepLinks 未钳制（条数/单项长度模型可控击穿上下文预算）；L12b——result-scorer.ts 每次打分 new ConformalHallucinationDetector（构造器对全量 factBase 重建 IDF 缓存，逐结果重复 tokenize）；L7——评估 proxyFetch ssrfGuard 默认开启风险面（仅评估）。
- **工具**：Read（search-engines.ts 521 行全文、result-scorer.ts 167 行全文、hallucination-detector.ts 568 行全文、proxy-fetch.ts 832 行全文、url-safety.ts 194 行全文、api-key-store.ts getEffectiveBaseURL 段、curl-fetch.test.ts / vram-probe-wiring.test.ts / crawl-filter-score-extract.test.ts 全文、审计 doc M9/L12/L7 条目）、Edit/Write（测试 1 新文件 + 源码 3 文件 6 处）、Bash（cp 备份/rm 备份/grep 调用方全量排查/bun test/bunx tsc/bun 回填脚本/git）。无子代理（切片要求串行）。
- **L7 评估**（只读排查，结论延期）：proxyFetch 调用方 16 文件全量 grep——①crawl/lightpanda-client.ts 以 proxyFetch 访问 CDP 端点 http://127.0.0.1:9222（url-safety.ts DEFAULT_CDP_URL 即回环，isSafeUrl 对 127./8、localhost、::1 全拦），默认开即断浏览器自动化；②router/model-router.ts（embeddings）、agents/computer-use-agent.ts、agent-evals/runner.ts、agents/kimi-code-agent.ts 的 baseURL 经 getEffectiveBaseURL/*_BASE_URL env 可解析到本地模型端点（127.0.0.1:9001 边缘 LLM 链路）；③local-llm/edge-embeddings.ts 走原生 fetch 不经本模块（不受影响亦不构成放行理由）。**结论：存在合法回环调用方，默认开启必破坏本地链路——维持 opt-in，不实施**；在 proxy-fetch.ts ssrfGuard 选项注释写明 opt-in 理由、受影响调用方清单与后续白名单建议（ssrfAllowHosts 或仅用户可控 URL 入口强制传 true），延期记录于本条目。
- **操作**（文件级）：
  1. 备份 search-engines.ts、result-scorer.ts、proxy-fetch.ts → `.tmp/backups/`（规则2，先通读各文件全文）。
  2. 红：新建 tests/crawl/audit-s4-network.test.ts——M9 静态断言（spawn proc.kill()/killTimer 兜底、CURL_PROC_TIMEOUT_MS=30_000 + CURL_PROC_KILL_GRACE_MS=5_000、SEARCH_ENGINE_TOTAL_BUDGET_MS=45_000 且接入 DDG/BingHtml 两循环≥3 处引用）；L12a 行为断言（clampDeepLinks 条数≤5/单项≤200/非字符串 JSON 序列化钳制 + BingEngine 注入 fetch 公共接口超长 deepLinks 被钳）与静态断言（常量并列定义 + 映射点 clampDeepLinks(r.deepLinks)）；L12b 静态断言（getSharedHallucinationDetector 存在、new ConformalHallucinationDetector 全文件唯一、DETECTOR_CACHE_MAX 上界）；curlFetch 正常路径回归（fake spawn exit 0 + -m 30 仍在）。首跑 fail（clampDeepLinks 导出缺失，症状与审计一致）。
  3. M9 绿两处：①curlFetch 默认 spawn 实现——killTimer 到 30s 先 proc.kill()（try/catch 容已退出），管道 arrayBuffer 挂 no-op catch 防竞速弃置后 unhandled rejection，Promise.race 硬上限 30s+5s 宽限仍未退出则抛错，finally 清理双定时器（-m 30 正常自退路径行为不变：立即退出→race 首臂胜出）；②DDG/BingHtml 重试循环加 startedAt 起点，attempt < attempts-1 且 elapsed ≥ SEARCH_ENGINE_TOTAL_BUDGET_MS(45_000) 时 logger.warn 并 break 放弃剩余重试返回已得结果（最坏 ~62s，较 92s 有界收紧；fetchImpl 注入路径 attempts=1 不受影响）。
  4. L12a 绿：常量 SEARCH_DEEPLINKS_MAX_ITEMS=5 / SEARCH_DEEPLINKS_ITEM_MAX=200 与 M6 常量并列；clampDeepLinks 导出（slice 5 条 + 字符串 slice 200、非字符串 JSON.stringify 后同样钳制）；Bing richSnippets 映射点改经 clampDeepLinks（unified-search.ts:198 透传点无需改动）。
  5. L12b 绿：模块级 detectorCache（Map，DETECTOR_CACHE_MAX=8 淘汰最旧）+ getSharedHallucinationDetector(factBase)——空 factBase 命中 "" 键（生产热路径 data-pipeline.ts:908 不传 factBase）；传入 [...factBase] 副本保证与逐次构造的快照语义一致（调用方后续原地修改不可见）；verify() 对状态只读（computeEvidence/computePValue 无写路径）已核实复用安全。scoreResult 公共接口行为不变。
...
## 2026-08-29 — fix(server)+fix(mcp): 审计 S5 切片 M11 chat 请求体校验 + L4 add 去重 + L6 死代码 + L8 HardFloor 字段扩展 + L9 依赖归位 + I6 注释修正 + L11 模型名边界（TDD）

- **任务**：审计修复切片 S5 服务与杂项（docs/reviews/2026-08-28-independent-full-audit.md，基线含 d0ff28e 的 S4 修复）：M11——routes/chat.ts handleChat 请求体零校验（`await ctx.req.json()` 后直接解构）；L4——ToolRegistry.add 直接 push 无去重（SDK 层 registerTool 遇重名抛错→启动期崩溃）；L6——getToolsByTags/getToolsMetaFiltered 零调用方死代码；L8——HardFloor 路径字段正则仅覆盖固定字段名（非常规字段携带路径绕过初筛）；L9——生产可达代码 src/codeindex/local-index.ts 导入 devDependencies 的 typescript（评估）；I6——src/testing/tool-count.ts 注释称 scripts/count-tools.mjs "已不存在"（实跑输出 188，事实错误）；L11——edge-client 默认 MiniCPM5-1B vs dre/config llmModel qwen3-1.7b-instruct 表面不一致（评估）。
- **工具**：Read（chat.ts 592 行全文、tool-registry.ts 235 行全文、local-index.ts 397 行全文、tool-count.ts 91 行全文、edge-client.ts 76 行全文、dre/config.ts 段、permissions.ts checkFilePermission 段、session-store/session-lineage/services-chat/engine/retrieveEvidence 段、search-route.test.ts / services-chat.test.ts / chat-tools.test.ts 测试模式参考、审计 doc 条目）、Edit/Write（测试 2 新文件 + 源码 6 文件）、Bash（cp 备份/rm 备份/grep 调用方全量排查/bun test/bunx tsc/bun install/临时探针 probe/git）。无子代理（切片要求串行）。
- **M11 schema 字段清单**（chatRequestSchema，导出供测试）：必填 `messages: Array<{role: string, content: string} & passthrough>`（passthrough 保留 name/多模态等扩展字段，元素校验与 /chat/stream isValidChatMessage 对齐）；可选 `taskType?: string`、`intent?: boolean`、`budget?: number | {maxTokens: number, preserveRecent?: number} & passthrough`（对齐 prepareChatContext options 的 `number | ComponentBudget` 契约）、`sessionId?: string`；未知顶层字段剥离（handleChat 仅消费上述字段，行为不变）；非法 JSON 与校验失败均 400 + 简明 error；空 messages 数组保持合法（对齐旧默认 `messages = []`）。handleAgentChat（:118）不在本审计条目证据范围（M11 引 :22），维持原样。
- **L4/L6/L8 证据**：L4——add 为全部 MCP 工具唯一注册收口，client-connector.ts:168 外部 MCP 重连按 `mcp_<server>_<tool>` 命名可重复 add（现网仅启动期调用一次，无重连循环；插件 disable 走 remove 后可再 add，全仓 grep 确认无依赖重复追加语义的调用方）→ 同名已存在 logger.warn + 跳过（幂等），返回 this 签名不变；188 静态计数（docs-consistency duplicates===[]）不受影响（静态扫描 src 文件字面名，与运行时 add 无交集）。L6——getToolsByTags/getToolsMetaFiltered 全仓（src/tests/scripts）grep 仅 tool-registry.ts 内部互引（:224），零外部调用方 → 删除。L8——各工具 inputSchema 参数名全量 grep（src/mcp/server/*、register-external-tools、src/tools/*）：path(20)/source(16)/filePath(14)/url(7)/target(5)/repoPath(5)/file(3)/cwd(3)/to(1)/destination(1)，其中 url 为未覆盖的路径承载字段（file:// 绕过向量）；filename 仅为 github-tools 返回字段非输入；repo(20) 为仓库名非文件路径不纳入 → 正则并集扩展为 camelCase/snake_case 常见变体 + url，注释声明仍非完备、依赖工具内部路径校验兜底（M12 呼应），不做深度值扫描。
- **L9 评估结论**：静态导入链实锤生产可达——main.ts:88 → memory/codegraph-index.ts:20 → codeindex/local-index.ts:10 `import ts from "typescript"`，`bun run src/main.ts`（npm start）运行时加载；且 `code:open`（package.json script + cli.ts:1244）经 cli.ts:703 惰性 import codegraph-index 同链路可达 → typescript 移入 dependencies（package.json 1 行 + bun install 刷 bun.lock，no changes 确认版本不变）。
- **I6**：scripts/count-tools.mjs 实际存在且实跑输出 188（`bun run scripts/count-tools.mjs` 验证）→ tool-count.ts 头注释删除"旧验证脚本已不存在"错误陈述，补写生成器事实（"bun run scripts/count-tools.mjs 直接调用本模块输出权威片段"）。
- **L11 决策**：两处服务于不同链路——edge-client 为边缘辅助链路（分类/改写/打标，EDGE_LLM_* env，:9001），dre/config 为 DRE 主推理链路（DRE_LLM_* env，:8080），env 前缀/端口/用途三者均独立，默认名不同是预期行为 → 按"依赖方向最小改动"择注释边界澄清（edge-client.ts 与 dre/config.ts 各加 2-3 行注释互指对方链路），不统一单一事实源（合并会造成 edge↔DRE 反向依赖）。
- **操作**（文件级）：
  1. 备份 chat.ts、tool-registry.ts、tool-count.ts、local-index.ts、edge-client.ts、config.ts、package.json、operations-log.md → `.tmp/backups/`（规则2，先通读各文件全文）。
  2. M11 红：新建 tests/routes-chat-validation.test.ts——mock model-router.js（绝对路径，与 services-chat.test.ts 同接缝；补 executeWithRole 因 handleChat 恒携 tools+role 走 runToolLoop）+ fake db，合法体 200/sessionId 透传、空数组 200、缺 messages 400、非数组 400、缺 content 400、schema 直解（passthrough/budget/剥离）。首跑 6 fail（0 pass，实锤零校验）。
  3. M11 绿：chat.ts 顶部定义并导出 chatRequestSchema（清单见上），handleChat 改 try/catch 解析 JSON + safeParse，失败 400 `Invalid request body — <path>: <message>`；解构去 `messages = []` 默认（schema 保证）。复跑 6 pass/0 fail。
  4. L4 红：新建 tests/tool-registry-dedup.test.ts——同名 add 两次断言 size===1 + warn 含工具名（logger.warn 实例属性 spy）、异名链式返回 this、remove 后可重注册。首跑 1 fail（重复 add 后 size===2 实锤）。
  5. L4/L6/L8 绿（tool-registry.ts 三处）：①add 入口同名检测 warn+skip；②删除 getToolsByTags/getToolsMetaFiltered；③HardFloor 正则扩展（path|file|filePath|file_path|target|destination|source|url|from|to|repoPath|repo_path|cwd|dir|dirPath|dir_path|folder|directory|absPath|abs_path|newPath|new_path|oldPath|old_path|destPath|dest_path|srcPath|src_path|outputPath|output_path，/i）+ 非完备性注释；新增 logger 导入。
  6. L9：package.json typescript ^5.3.0 devDependencies → dependencies，bun install 刷新 bun.lock（Checked 144 installs，no changes）。
  7. I6 + L11：tool-count.ts 注释修正；edge-client.ts / dre/config.ts 边界注释（见 L11 决策）。
  8. test:full 名单补入两个新测试文件（package.json script 追加）。
  9. 本条目 docs/operations-log.md。
...
## 2026-08-29 — fix(agent-evals)/fix(router): 审计强化 T6 P1-3 评测沙箱执行 + P1-4 thompson 观测有界与空 arms 降级（TDD）

- **任务**：联合审查 High（docs/reviews/2026-08-29-joint-verification-audit.md §4 B2-M3/M4）——①B2-M3：src/agent-evals/external.ts runPython 将 LLM 生成的 Python 落盘后以当前用户权限直接执行（timeout 20s），可访问文件系统/网络；②B2-M4：src/router/thompson-router.ts 观测无界增长（内存 obs 无限 push + SQLite thompson_observations 只增不删），getEffectiveParams 每次 route 全量遍历线性变慢，且 main.ts:229 以 arms:[] 构造时 route() 抛 "TS selected no arm"。spec §2 P1-3/P1-4，计划 docs/superpowers/plans/2026-08-29-audit-hardening-plan.md Task 6。
- **工具**：Read（external.ts/docker-sandbox.ts/thompson-router.ts/process-sandbox.ts/path-safety.ts/spawn-env.ts 全文 + route() 调用方与既有测试通读）、Write（2 新测试文件）、Edit（2 源文件 + 1 既有测试 + package.json）、Bash（cp/rm 备份、bun test、bunx tsc、git）、bun 脚本（本条目追加与 hash 锚点回填，未用 sed）。无子代理（串行约束）。
- **操作**（文件级）：
  1. 备份 4 文件 → .tmp/backups/（规则2，先通读全文），验证通过后删除。
  2. P1-3（src/agent-evals/external.ts）：执行统一改经 SandboxProvider（默认 docker-sandbox；ExternalLoadOptions 新增 sandbox 注入缝）；脚本改写至项目内 .tmp/external-eval-runs/<run>/generated_code.py 并作为沙箱 cwd 挂载（docker-sandbox 挂载白名单会拒绝 os.tmpdir 宿主用户目录首段，容器内经 /workspace 相对引用）；命令由 shellQuoteArg(linux) POSIX 单引号构造（脚本内容在文件内，命令面无注入面）；保持 20s 超时语义（timeoutMs: 20_000）与默认禁网（networkAccess: false）；解释器候选改为容器语义 [python3]/[python]，127/not found/ENOENT 才尝试下一候选，非零退出（语法/断言失败）直接返回；沙箱不可用 → fail-closed 返回 "skipped: <provider> sandbox unavailable"，绝不回退宿主直跑；移除 node:child_process 依赖，runDir 用后即删。
  3. P1-4（src/router/thompson-router.ts）：新增导出常量 MAX_OBSERVATIONS_PER_ARM=500；reportFeedback 内存 obs 超 500 裁最旧（splice）；saveObservation 写入侧增量裁剪（INSERT 后 DELETE WHERE arm_id=? AND id NOT IN (SELECT id ... ORDER BY id DESC LIMIT ?)，idx_obs_arm 索引 O(log n)）；loadState 启动侧有界加载（ORDER BY id DESC LIMIT 500 后 reverse）+ 一次性清理历史超限行；顺带修复既有潜伏 bug——saveObservation 的 INSERT 此前缺 created_at（表 NOT NULL 无默认值）导致观测落库一直静默失败（调试 DB 测试 count=0 时以 bun 真实路径复现，NOT NULL constraint failed 堆栈定位），现补 Date.now()；route() 空 arms 早返回确定性降级 RoutingDecision（arm.id=""、confidence 0、samples []、reason "degraded: no arms configured"），不抛错；新增 getObservationCount(armId) 观测可观测性方法。
  4. tests/agent-evals/external-benchmarks.test.ts：执行缝随 P1-3 适配——新增宿主执行 fake 沙箱 makeHostSandbox（解析 POSIX 单引号命令回宿主 spawn），三个 verify 判定测试改注入 sandbox（pythonCmd 与 exit0/非0/解释器缺失断言语义不变），不依赖真实 Python/docker。
  5. 新建 tests/external-eval-sandbox.test.ts（3 测试：静态断言 external.ts 无 node:child_process/Bun.spawn/spawn( 且执行走 sandbox.execute+available()；沙箱不可用 → passed:false 且 reason 含 skipped 且 execute 零调用即无子进程执行；沙箱可用 → 脚本落在 .tmp/external-eval-runs 挂载目录内、timeoutMs=20000、networkAccess=false、命令含脚本名）；tests/thompson-observability.test.ts（3 测试：1001 次 reportFeedback 后 getObservationCount=500 且 getArmStats alpha=501 证明最旧被裁（未裁应为 1002）；DB 模式 520 次反馈后 sqlite 直查 thompson_observations count=500；空 arms route() 不抛且返回 arm.id=""/samples 空/confidence 0/reason 含 degraded）。
...
- **Commit**：feat(memory): P0-B 跨会话记忆闭环（会话自动归档+bootstrap 召回） — f6f91f2

## 2026-08-29 — feat(safety): P0-C 幻觉防线接火（请求级 factBase + verify 双缝接线）

- **任务**：P0 提升迭代 Task C（docs/superpowers/specs/2026-08-29-p0-lift-design.md §C）：修复审计判定的"幻觉检测器橡胶图章"——hallucinationDetector 实例化即弃（main.ts:228 factBase=[]）、calibrate 零调用 → pValue 恒 1.0 永不判幻觉；chat 已有检索证据（knowledge+codegraph）却未用于校验响应。
- **工具**：Read/Bash（通读 4 目标文件全文、检索证据结构取证、回归测试、bunx tsc、git）、Write/Edit（实现与新测试）。全程无子代理（任务要求串行）。AGENTS 规则 2 全程执行：备份 .tmp/backups/ → 通读全文 → 最小改动 → 验证 → 删备份。
- **操作**（文件级）：
  1. src/memory/hallucination-detector.ts：追加 P0-C 接线辅助段（纯函数，领域内聚于既有深模块）——buildFactBaseFromRetrieval（knowledge/codegraph 检索文本 → FactEntry[]：knowledge 按空行分块并从 "• [source]" 提取来源、剔除检索头行，confidence 0.7；codegraph 分块 source=codegraph、confidence 0.9；条目文本 400 字符截断、总量 64 条上限）；buildFactBaseFromEvidence（DRE metadata.evidence 的 string[] / {text,confidence?,source?}[] → FactEntry[]，非法形态忽略）；classifyHallucinationVerdict（三档分类 accepted/anomalous/hallucination：isHallucination 优先、未校准下以 confidence < 0.1 判 anomalous）；assessStatement（请求级校验入口：每次调用独立 detector 实例、无共享状态、不污染 main.ts 全局单例；空证据/空陈述返回 null 不做无据判定，避免空 factBase 全量误标；可疑时 logger.warn 打标记录；陈述 2000 字符截断）。校准债声明（本段注释 + 本条目）：真实 calibrate 需要 (statement, isFact) 标注对积累，P0 不做——P0 交付"未校准运行的可观测校验"（pValue 恒 1.0，可疑判定仅由证据相似度驱动），calibrate 数据积累与真实校准为 P1。
  2. src/services/chat.ts：PreparedContext 新增 evidence: FactEntry[]（本次请求检索证据，纯函数构建于 prepareChatContext 返回前；未走检索路径为空数组）；knowledge 分支捕获 kr.context 至 knowledgeContext 局部变量。
  3. src/routes/chat.ts（缝①，两处）：handleChat 在 executeChat 后对 result.content 跑 assessStatement(evidence, ...)，结论以最小兼容字段 `_hallucination: { pValue, verdict, isAccepted }` 写入 /chat 响应 JSON（null = 无检索证据校验未运行）；handleChatStream 在 done 帧对 ev.content 同样校验并随 done SSE 事件下发 _hallucination。可观测不阻断：不改任何既有返回字段与降级行为。
  4. src/dre/engine.ts（缝②）：cloudConsciousnessStep 在 parseCloudDecisionOrThrow 之后，把 input.metadata.evidence 经 buildFactBaseFromEvidence 转事实库，对 decision.content 跑 assessStatement；低置信（verdict 为 anomalous/hallucination）时打标记录并抛错 → consciousnessStep 既有降级链走 L3 ruleBased 路径（与 M11 "云端坏输出与本地同级"语义一致），返回结构不变；无证据（当前所有调用方未传 metadata.evidence）时跳过校验，行为与之前完全一致。
...
- **Commit**：feat(dre): P1-S1 检索唤醒（routes/search dre 段 + vault 回退链 DRE 补充） — 71423ba5bb0b293192ae63b702d8c1d868ec1f11

## 2026-08-29 — feat(memory): P1-S2 中文 bigram 召回（内存引擎 CJK bigram + memory_notes_fts trigram 迁移带降级）

- **任务**：P1 提升迭代 S2（docs/superpowers/specs/2026-08-29-p1-lift-design.md §S2）：修复审计判定"FTS5 unicode61 把连续中文当单 token，语序改写即漏召回——词法范式内最大单点损失"。层1（零迁移）：deterministic-search tokenize 引入 CJK bigram，索引/查询同函数天然一致；层2（带迁移降级）：memory_notes_fts unicode61→trigram 迁移（实测 bun:sqlite SQLite 版本 **3.53.0 ≥ 3.34，trigram 支持**），失败回退保留旧表；KAL queryVault 与 SQLiteMemory.search 查询侧适配（trigram 最小 3 字符——<3 字 CJK 短词走 memory_notes LIKE 兜底腿，unicode61 库保持旧行为）。
- **工具**：Read/Bash（通读 3 目标文件全文、trigram 语义实证探测、bun test/tsc、git、真实库副本演练）、Write/Edit（实现与新测试）。全程无子代理（任务要求串行）。AGENTS 规则 2 全程执行：备份 .tmp/backups/ → 通读全文 → 最小改动 → 验证 → 删备份。
- **操作**（文件级）：
  1. src/memory/deterministic-search.ts（层1）：tokenize 对含 CJK 的分段先切出连续中文串/ASCII 子串，中文串切相邻二字组（"机器学习"→机器/器学/学习），孤立单字中文保留（单字查询不回归），ASCII 词保持原样（纯英文行为不变）；tokenizeCache 复用不变。参考实现 self-evolve/engine.ts:53 经架构断言权衡（其 STOPWORDS 语义会改变英文路径行为）后本地实现，不跨目录复用。
  2. src/memory/sqlite-memory.ts（层2）：三个同步触发器 SQL 提取为模块常量 MEMORY_NOTES_TRIGGER_SQL（initSchema 与迁移重建共用）；initSchema 建表改 tokenize='trigram'（trigram 建表失败时降级 unicode61 并 warn）；新增导出 migrateMemoryFtsToTrigram(db)——事务内 删触发器→旧表 RENAME 保底→trigram 重建→'rebuild' 全量回填（external content）→行数与源表校验→删保底表→重建触发器，任一步失败 ROLLBACK 旧表触发器原样保留（检索降级不中断）；构造器启动时自动迁移；search() 拆词——trigram 库 <3 字 CJK 短词不进 MATCH（恒空）改走 memory_notes LIKE 兜底腿（ESCAPE 转义、opts 过滤与 FTS 腿一致，结果去重合并后统一 tagFilter/slice），unicode61 库保持全部词进 MATCH 的旧行为。
  3. src/kal/knowledge-access-layer.ts：sanitizeFTS5 加 minWordLength 参数（默认 1 旧行为；trigram 传 3 过滤短词）；新增 ftsUsesTrigram()（sqlite_master 探测一次并缓存）；queryVault 拆 FTS MATCH 腿 + 短词 LIKE 兜底腿（清洗后词仅含 \w/CJK 不含 LIKE 通配符，无需转义；ORDER BY mn.id ASC 确定性），按 path 去重合并。
  4. 新建 tests/cjk-bigram-recall.test.ts（层1 三测试：语序改写"学习机器"命中"机器学习笔记"/单字"学"不回归/纯英文命中与不命中不变）；新建 tests/memory/fts-trigram-migration.test.ts（7 测试：unicode61→迁移→SQL 含 trigram 且行数一致且子串可召回；幂等再跑 migrated=false；Proxy 模拟 rebuild 抛错→旧表+触发器保留数据可查；SQLiteMemory 构造即迁移存量库且短词 LIKE 兜底召回+英文不回归；KAL trigram 下 ≥3 字子串 MATCH 召回/<3 字短词 LIKE 兜底召回/无关短词不虚报）。
- **验证**：TDD 红→绿：层1 先红（查"学习机器" 0 命中）→ 后 3 pass/0 fail；层2 先红（SyntaxError: Export 'migrateMemoryFtsToTrigram' not found；KAL 短词 case fail）→ 后 10 pass/0 fail（28 expect）。回归分批全 0 fail：tests/memory/ 31 pass；kal-references+kal-deterministic-order 19 pass；deterministic-search+tie+vault-reindex 20 pass；dre-retrieval-engine+chat-memory-loop 42 pass；architecture-integrity **24 pass/0 fail**。bunx tsc --noEmit 0。真实库副本演练：data/agent.db（174 行）已为 trigram → 迁移正确幂等跳过（migrated=false）。事实记录：tests/stress/audit-regression-stress.test.ts 超时风暴 1 fail 为**存量失败**（HEAD 干净 worktree 复现，Actor 并发计数断言，与本切片 memory/KAL 改动无关）。
- **Commit**：feat(memory): P1-S2 中文 bigram 召回（内存 CJK bigram + FTS trigram 迁移带降级） — fd62697

## 2026-08-29 — feat(agents): P1-S3 主链路结构化输出收紧（四处解析点接 zod schema）

- **任务**：P1 提升迭代 S3（docs/superpowers/specs/2026-08-29-p1-lift-design.md §S3）：修复评估报告"过松"项——主链路 JSON 解析靠 extractJson+手写字段检查、无 schema。四处解析点接 zod：intent-enhancer 云意图解析、edge preflight 合并解析、risk-monitor 复核解析、DRE parseCloudDecisionOrThrow。红线：解析失败降级行为与现状一致（失败=既有 return null/回退路径，只是判定更早更明确）。
- **工具**：Read/Bash（通读 4 目标文件全文与既有测试、zod 3.25.76 版本实证、bun test/tsc、git）、Write/Edit（schema 接缝与新测试）。全程无子代理（任务要求串行）。AGENTS 规则 2 全程执行：备份 .tmp/backups/ → 通读全文 → 最小改动 → 验证 → 删备份。
- **操作**（文件级）：
  1. src/agents/intent-enhancer.ts：新增导出 intentResponseSchema（intent 枚举 6 值 + confidence number.min(0).max(1).finite() + reason string optional，passthrough）；parseClassifierResponse 两处手写 typeof 检查替换为 schema.safeParse；调用方 VALID_INTENTS 检查保留（schema 已保证枚举，双重防线）。
  2. src/services/chat-preflight.ts：新增导出 mergedPreflightSchema（rewritten trim 后 min(1) + intent trim 后枚举 + confidence finite；transform 保持旧"先 trim 再判空/枚举"语义）；mergedEdgePreflight 手写 typeof 检查（含 confidence 错型默认 0）替换为 safeParse（失败 null 回退串行不变）；原 VALID_INTENTS 常量删除（唯一消费点已被替换）。
  3. src/agents/risk-monitor.ts：新增导出 riskReviewSchema（dangerous boolean + reason string optional）；reviewWithDecisionModel 手写 typeof dangerous 检查替换为 safeParse（失败 null=复核不可用，fail-closed/fail-open 语义不变）。
  4. src/dre/constraints.ts：新增导出 dreDecisionSchema（action 枚举 + content string + confidence min(0).max(1)，与 DRE_DECISION_SCHEMA/旧手写校验等价、强度只增不减）与 DreDecision 类型；isDreDecision 改为 safeParse 守卫；parseCloudDecisionOrThrow 流程与错误消息逐字节不变（JSON.parse→isDreDecision→CloudDecisionInvalidError，返回原始解析对象保留多余键）。
  5. 新建 tests/zod-parse-seams.test.ts（8 测试，每处 2 例：合法输入通过且结果同旧——代表输入取自各域既有测试；畸形输入 schema 拒绝——端到端降级由既有测试覆盖）。
- **验证**：TDD 红→绿：新测试先红（SyntaxError: Export 'dreDecisionSchema' not found）→ 实现后 8 pass/0 fail（41 expect）。回归分文件全 0 fail：intent-enhancer 30 pass；chat-preflight-parallel 13 pass；risk-monitor 20 pass；dre-constraints+unit/cloud-decision-guard 8 pass；dre-core-modules（engine decide 钩子消费方）96 pass；architecture-integrity **24 pass/0 fail**（zod 为 npm 包不构成目录边）。bunx tsc --noEmit 0。降级一致性证据：intent-enhancer「非法意图/非 JSON 回退 baseIntent」、chat-preflight「意图非法/缺字段/非 JSON → null」、cloud-decision-guard「非 JSON/缺字段抛 CloudDecisionInvalidError」既有断言全数通过；置信度错型（preflight 旧默认 0）/非有限数等边界为"判定更早更明确"的收紧点，无既有测试依赖，下游均为安全回退。
- **Commit**：feat(agents): P1-S3 主链路结构化输出收紧（四处解析点接 zod schema） — 40651944262a4f228dcc1d3484327728a9d7f2da

## 2026-08-29 — feat(router): P1-S4 thompson 学习回路（arms 填充 + 反馈接线 + 平级 tie-break）

- **任务**：P1 提升迭代 S4（docs/superpowers/specs/2026-08-29-p1-lift-design.md §S4）：修复评估报告断链"模型路由纯静态优先级，学习信号从不反哺选模"。main.ts:235 arms:[] → 由模型注册表填充；reportFeedback 零调用 → execute 成败记录点接线；同优先级候选 ≥2 平级组用 thompson 采样 tie-break（不改不同优先级排序；未注入/空 arms/组内 arm 未注册/采样异常 → 现状排序逐字节兼容）。
- **工具**：Read/Bash（通读 model-router.ts、thompson-router.ts、main.ts、model-capability-registry.ts、models/{registry,providers,types}.ts 全文，候选优先级实证，bun test/tsc、git）、Write/Edit（实现与新测试）。全程无子代理（任务要求串行）。AGENTS 规则 2 全程执行：备份 .tmp/backups/ → 通读全文 → 最小改动 → 验证 → 删备份。
- **操作**（文件级）：
  1. src/router/model-router.ts：新增导出 ThompsonLearningPort（结构化最小消费端口：getArmIds/route/reportFeedback，ThompsonRouter 公共接口天然满足，测试可注入 fake，类型仅 import type 自同目录 thompson-router.js）；新增导出 buildThompsonArms()——listAllModels()（静态 UNIFIED_REGISTRY + 用户扩展）按 id 去重映射 RouterArm{id: 模型注册表 id, model, provider, alpha=beta=1}（事实说明：spec 文本"id=providerId"按注册表唯一模型 id 落地——arm 需与 execute 候选一一对应，若按 provider 键两 deepseek 模型会互相覆盖且 tie-break 无法按候选反馈）；compareRouteCandidates 提取（语义与原内联比较器一致：轻任务 isFree 优先、其余按 effectivePriorityForRateTier 升序）；MultiPlatformRouter 新增私有 thompson 端口 + setThompsonRouter 注入口（null=摘除）；applyThompsonTieBreak——相邻比较器相等即平级组（等价于 (priority, isFree) 相同连续段），组内 arm 全注册时 await route() 一次、按 samples 值降序重排组内顺序，异常 logger.debug 吞掉保持静态序，不改组间排序；execute() 排序行改为 await applyThompsonTieBreak([...candidates].sort(compareRouteCandidates), ...)；成功 trackCall 后 reportThompsonFeedback(model.id, true)、失败 catch 内 trackCall 后 reportThompsonFeedback(model.id, false)（失败语义=trackCall success:false，success 判定源=callProvider resolve/reject；缓存命中两路径不计反馈——无真实模型调用信号），reportThompsonFeedback 异常吞掉+debug 日志不破坏主流程。chatStream/tool 路径未动。
  2. src/main.ts：import buildThompsonArms 与 router 别名 modelRouter；mathContext.thompsonRouter 构造 arms: buildThompsonArms()（minSamples 5 / inMemory true 不变）；mathContext 构造后 modelRouter.setThompsonRouter(mathContext.thompsonRouter) 组合根接线。
  3. 新建 tests/thompson-routing-learning.test.ts（7 测试，零真实网络——fetch 全局 spy + 测试 key 注入，永久失败路径放文件末尾防拉黑串扰）：①arms 填充（注册表全量→数量/字段/均匀先验）；②反馈接线（成功→reportFeedback(armId,true) 恰一次；失败路径→所有记录均 false 且含首个候选）；③tie-break 学习（同优先级 gpt-5.4-pro/claude-opus-4.8 平级组，真实 ThompsonRouter（工厂默认 decayFactor=0.95）固定反馈偏袒静态第二位 B 30 胜/负后，20 次 execute 统计 B ≥15/20——红→绿证据：实现前该断言 0/20）；④降级（未注入→恒静态序首个；空 arms fake→现状排序且反馈不崩溃）；组合根接线静态断言（main.ts 含 arms: buildThompsonArms() 与 setThompsonRouter(mathContext.thompsonRouter)）。
- **验证**：TDD 红→绿：新测试先红（SyntaxError: Export 'buildThompsonArms' not found）→ 实现后 7 pass/0 fail；中途发现测试直接 new ThompsonRouter 漏传 decayFactor 产生 NaN 静默破坏采样（route 抛错被 tie-break 捕获降级）——改用 createThompsonRouter 工厂默认后绿，该坑仅测试侧，thompson-router 本体未改。回归分文件全 0 fail：thompson-stress 5 pass；property-based 40 pass；model-router 8 pass；flat-router 9 pass；tests/router/ 41 pass；thompson-observability 3 pass；router-cost-routing 2 pass；scene-router 13 pass；services-chat 5 pass；routes-chat-validation 6 pass；architecture-integrity **24 pass/0 fail**（同目录 import type 不构成目录环）。bunx tsc --noEmit 0。新测试复跑一次稳定（统计断言 b 胜率波动远离 15 阈值）。
- **Commit**：feat(router): P1-S4 thompson 学习回路（arms 填充 + 反馈接线 + 平级 tie-break） — 16e006098d9d114aa5b4e329c2a21ddfa24df6c3

## 2026-08-29 — feat(safety): P1-S5 校准数据积累（双缝 verdict 落库 + calibrateFromStored 保守自动校准）

- **任务**：P1 提升迭代 S5（docs/superpowers/specs/2026-08-29-p1-lift-design.md §S5）：修复 P0-C 校准债——双缝 verify（缝① chat / 缝② DRE）已运行但 verdict 未持久化，calibrate（hallucination-detector）零数据 → 共形 p-value 判别力锁死。①新表 hallucination_verdicts（migrate.ts 纳管）；②两缝 verdict 落库（异步吞错不阻塞响应）；③calibrateFromStored 保守自动校准（极化对 <50 跳过），main.ts 启动接线一次。
- **工具**：Read/Bash（盘点遗留改动、通读 hallucination-detector.ts/migrate.ts/dre/engine.ts/routes/chat.ts/main.ts 相关全文与既有测试、bun test/tsc、git）、Edit/Write（实现与新测试）。全程无子代理（任务要求串行）。AGENTS 规则 2 全程执行：备份 .tmp/backups/ → 通读全文 → 最小改动 → 验证 → 删备份。
- **操作**（文件级）：
  1. 新建 src/db/hallucination-verdicts.ts：ensureHallucinationVerdictsTable（幂等，DDL 与 migrate.ts 纳管版一致，含 statement 原文列——calibrateFromStored 构建 CalibrationPair 需要它，为任务书表结构的最小必要扩展）与指纹/摘要索引；recordHallucinationVerdict（吞错：任何失败仅 logger.debug 不上抛——调用方位于响应路径/推理降级链上）；computeStatementDigest=sha256(statement.slice(0,2000))；computeEvidenceFingerprint=sha256(排序去空前 8 条证据以换行拼接，与输入顺序无关)；readHallucinationVerdicts；calibrateFromStored(db, detector, minPairs=MIN_CALIBRATION_PAIRS=50)。
  2. src/db/migrate.ts：纳管同一 DDL（CREATE TABLE IF NOT EXISTS + evidence_fingerprint/created_at 两索引）。
  3. 缝① src/routes/chat.ts：非流式 _hallucination 判定处与流式 done 帧校验处，routes→db 直调 recordHallucinationVerdict（seam:"chat"；依赖方向与 session-store 既有 routes→db 一致，architecture-integrity 无此向禁令，_hallucination null=无证据未判定不落库）。
  4. 缝② src/dre/engine.ts：DREConfig 新增可选端口 recordVerdict?: (rec) => void（与 P0-C hallucinationGate 同注入模式，dre 包零 db 直引）；cloudConsciousnessStep 的 assessment 判定后调用，**抛错前先落库**（可疑判定是最有价值的校准数据，不因 L3 降级链中断而丢失）。
  5. src/main.ts 组合根：DEEPSEEK 分支 setDreHostOverrides 注入 recordVerdict 适配器（db 于下方启动段创建，闭包惰性引用 + try/catch 兜底未初始化时序；evidence 经 buildFactBaseFromEvidence 归一为 text 列表，seam:"dre"）；启动段一次性 calibrateFromStored(db, mathContext.hallucinationDetector)（整体 try/catch 吞错不阻断启动；有结果 logger.info 含 n/meanScore/scoreDistribution，不足 logger.debug）。
  6. 新建 tests/hallucination-calibration.test.ts（9 测试）：①落库与读取（digest=sha256(slice(0,2000))、fingerprint 与证据顺序无关、is_accepted 0/1、seam 标记、record 吞错）；②<50 跳过（返回 null 且 detector.isValid()=false；同指纹组无极化等效不足跳过）；③≥50 造数（30+30 极化对→calibrate 被调、quality.n=60、isValid=true；minPairs 参数可覆盖默认 50）；④建表幂等（重复 ensure 不抛错不丢数据）。
...
- **Commit**：feat(safety): P2-S4 HITL 真值标注管道（label 列 + hallucination_feedback 工具 + calibrate 真值优先 + tool-count 189） — d549c64

## 2026-08-30 — fix(dre): P2-S5 M10 云端降级上下文补全（本地工作记忆随行云端 prompt）

- **任务**：docs/superpowers/specs/2026-08-30-p2-closeout-design.md §S5，审计 M10（cloudConsciousnessStep 降级只发 input.observation，本地工作记忆不随行，降级后行为不一致）：云端降级 user prompt 注入本地工作记忆摘要（最近 N 条 + 最近反思结论，≤2KB），记忆不可用/为空时逐字节现状直发。
- **工具**：Read/Bash（通读 engine.ts/consciousness/stream.ts/ports/cloud-caller.ts/规格 §S5/hallucination-wiring 测试组合根模式）、Write/Edit（TDD 测试与实现、最小改动）、Bash（bun test 定向/tsc/git）、bun 脚本（hash 回填，未用 sed）。全程无子代理（任务要求串行）。AGENTS 规则 2（备份 .tmp/backups/ → 通读全文 → 最小改动 → 验证 → 删备份）与规则 7（垂直切片红→绿）全程执行。
- **操作**（文件级）：
  1. src/dre/engine.ts：①新增命名常量（带注释）CLOUD_CONTEXT_DEFAULT_MAX_ENTRIES=5（可配，0=禁用）/CLOUD_CONTEXT_MAX_ITEM_CHARS=200/CLOUD_CONTEXT_MAX_BYTES=2048/CLOUD_CONTEXT_HEADER="[Local working memory context]"；②新增导出纯函数 buildCloudMemoryContext（条目时间倒序、空白跳过、每条截 200 字符、UTF-8 字节预算 ≤2KB 整行淘汰+单条硬截断 sliceUtf8Within 不产生半字符；reflection: 行随行最近反思结论）与 composeCloudUserPrompt（有摘要=observation+"\n\n"+头+"\n"+摘要；无摘要逐字节现状）；③DREConfig 加 cloudContextMaxEntries?（M10 注入条数上限，默认 5）；④私有 lastReflectionSummary 字段 + handleReflection 内记录（issues+lessons 拼接，截断在 build 内做）；⑤私有 cloudMemoryContext()（try/catch 吞错返回 null，logger.warn 留痕）；⑥cloudConsciousnessStep 的 caller.call user 改 composeCloudUserPrompt 注入（system=DRE_DECISION_SYSTEM 与返回结构不变）。
...
- **Commit**：feat(kal): W5 KAL queryKG FTS5 trigram（独立虚拟表+rowid 触发器+幂等回填+MATCH/LIKE 并集，排序红线保持） — fbb47c2

## 2026-08-30 — refactor(dre): W8 SearchPort 端口分层（pipeline 去 crawl 静态依赖，M13 闭合 + L1 盲区收口，注入优先防 mock 绕过）

- **任务**：按落地形态审核 `docs/knowledge/w5-w8-landing-form-audit-2026-08-30.md` §3 落地 W8——dre 侧 SearchPort 端口接口 + crawl 结构兼容适配 + 默认值注入：pipeline.ts 移除 `:16` crawl 静态 import（M13 反向依赖），改经 `search-port.ts` 端口；构造签名不变（`opts.searchAgg?` 类型换 SearchPort，engine.ts:224 组合根不破坏）；architecture-integrity L1 补 dre→crawl 禁令（豁免端口文件），闭合盲区。规避 D1 缺陷①（注入 mock 恒生效，不静默绕过打真实网络）。
- **工具**：Read（通读 pipeline.ts/search-engines.ts SearchAggregator+searchMulti 签名/architecture-integrity L1/dre-stage2-webverify 测试模式/engine.ts:224 组合根）、Write（TDD 红测试+端口文件）、Edit（pipeline 三处最小改动+L1 追加断言）、Bash（bun test 定向/tsc/test:full/备份删除）。无子代理（TDD 串行）。AGENTS 规则 2（备份→通读→最小改动→验证→删备份）与规则 7（红→绿）全程执行。
- **操作**（文件级）：
  1. 新建 `src/dre/pipeline/search-port.ts`：`SearchPort` 接口（`searchMulti(opts, engines?)`，与 SearchAggregator 结构兼容）+ re-export `SearchOptions/SearchEngineResult/SearchFetch` + `defaultSearchPort()`（返回 crawl 模块单例 searchAggregator，同步）+ `searchAggregatorFromFetch(fetch)`（内部 new SearchAggregator(fetch)）。L1 唯一豁免静态 import crawl 的 dre 文件。
  2. `src/dre/pipeline/pipeline.ts`：`:16` import 换为 `./search-port.js`（defaultSearchPort/searchAggregatorFromFetch/type SearchPort/SearchEngineResult/SearchFetch）；字段 `searchAgg: SearchPort`；构造 opts `searchAgg?: SearchPort`；赋值 `opts.searchAgg ?? (opts.searchFetch ? searchAggregatorFromFetch(opts.searchFetch) : defaultSearchPort())`——注入优先，默认值走端口。调用点 `searchAgg.searchMulti` 签名不变。
  3. `tests/architecture-integrity.test.ts` L1 追加 `it("src/dre 除 search-port.ts 外不得引用 crawl/（W8 M13 收口）")`：walk src/dre，正则 `from "..../crawl/` 或 `import(".../crawl/"`，豁免 `search-port.ts`，保留既有 dre→router 断言。
...
## 2026-08-31 — feat(self-evolve): 自动 evolve 触发器（chat 轮末 fire-and-forget，闭环学习侧接线）

- **任务**：评估报告（`docs/knowledge/agent-decision-chain-assessment-2026-08-29.md` §一/§四）识别的断链"real-usage 轨迹仅 CLI 手动触发"——采集侧已自动（captureRealUsageTrace 接线 chat 三处 → `data/real-usage-traces.jsonl`），学习侧（evolveFromRealUsage → selfInduce → promoteInductionsToSkills）只在 `bun run src/agent-evals/real-usage.ts --evolve` 时跑。本任务闭合该半闭环：新增 `maybeAutoEvolve` 控制器，每次 chat 交换后 fire-and-forget 廉价检查，冷却（默认 30min）+ 新增轨迹阈值（默认 30）+ 并发门闩达标时自动执行既有 evolve 链路（复用，不改内部）。**默认 OFF（用户确认）**：`AXIOM_SELF_EVOLVE_AUTO=1` 才激活，未开启时零读零写。
- **工具**：Read（real-usage.ts/env.ts/skill-quality.ts 原子写模式/real-usage.test.ts 接缝）、Write（TDD 红测试 + auto-evolve.ts）、Edit（chat.ts 三处接线 + 修 tsc 报错）、Bash（bun test 红→绿/tsc/test:smoke）。无子代理。AGENTS 规则 2（备份 chat.ts → 通读 → 最小改动 → 验证 → 删备份）与规则 7（红→绿）全程执行。
- **操作**（文件级）：
  1. 新建 `src/agent-evals/auto-evolve.ts`：`maybeAutoEvolve(deps?)` + 全注入 `AutoEvolveDeps`（now/enabled/minNewTraces/cooldownMs/statePath/getNewTraces/evolve，默认真实现）。控制流：enabled 开关（默认 false，关闭即返回）→ 模块级 `running` 布尔首个 await 前置位（真实并发门闩）→ 读 state（缺失/损坏降级全 0，W3 不抛）→ **冷却短路**（距上次 < cooldown 直接返回，不做昂贵全量轨迹读——默认窗口内每轮 turn 只付一次小 JSON 读）→ `getNewTraces`→ 新增数 < 阈值 → `insufficient-new` → `evolve` → tmp+renameSync 原子写 state（`data/auto-evolve-state.json`，gitignore 覆盖）。evolve 抛错：warn + **仍推进 state**（防下一轮 turn 热重试同一失败）→ `reason:"error"`。
