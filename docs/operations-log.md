...
- **Commit**：fix(orchestrator): 审计 H3 orchestrator_execute_plan 步骤超时兜底（timeoutMs 透传 + DEFAULT_STEP_TIMEOUT_MS=120000）（含 `src/mcp/server/orchestrator-tools.ts` + `tests/orchestrator-plan-timeout.test.ts` + `docs/operations-log.md`） — hash 待回填
