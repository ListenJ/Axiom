...
- **Commit**：`docs+feat: 澄清mineru零LLM边界，双端探针聚合与DRE调用链超时/降级`（含 `docs/KNOWLEDGE-BASE.md` + `docs/LIMITATIONS.md` + `scripts/audit/dual-probe.ts` + `src/router/provider-caller.ts` + `tests/unit/docs-consistency.test.ts` + `tests/rigorous/real-links-memory-knowledge-prompt.test.ts` + `docs/operations-log.md`，`internal211` 待回填）
