...
- **Commit**：`pending` `fix(mcp): 清理不存在的sqlite声明与失效包名，补超时可配与降级warn`（含 `mcp-servers.yaml` + `client-connector.ts` + `main.ts` + `mcp-config.test.ts` + `operations-log.md`，`internal211` pending）
