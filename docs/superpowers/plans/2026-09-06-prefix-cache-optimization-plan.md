...
| P1-C | **prompt 前缀纪律接入主路径** | 切片①✅ CACHE_BOUNDARY marker 去随机化（内容 hash 派生，跨重启字节稳定，d187124）；切片②✅ provider-caller 请求边界工具列表稳定排序（orderToolsForCache，d187124）；切片③**押后**：prompt-pool 静态前缀整体接入 router 主路径——重接线会全局替换 router 消费方 system prompt（行为变更），将作废本迭代刚建立的 66 任务基线与回归防线参照，需独立切片 + 行为差异评估 + 重基线后再施工 | S（①②）/ M（③） | ①② 完成（2026-09-06）；③ 押后 |
