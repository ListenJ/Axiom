...
## 六、需用户确认的关键决策（T3 门禁，动工前必答）

- **D1 范围**：MVP 只做缺口 A（交叉验证）+ B（审核回环），还是含 C（用户配置）？建议 A+B。
- **D2 交叉验证模型来源**：复用 `Dispatcher`+`executeWithRole`（现役 registry 角色），还是新定义"验证角色组"？建议复用。
- **D3 分歧裁决策略**：多数投票 / 指挥模型仲裁 / ValidationPipeline 硬校验？建议"多数投票 + 平票交仲裁角色"。
- **D4 触发条件**：全量交叉验证（安全但贵）vs 关键结论触发（S6）？建议后者，默认阈值待校准。
