...
/**
 * zod 决策 schema（S3 结构化输出收紧）：与 DRE_DECISION_SCHEMA 一一对应，
 * 必填/枚举/区间与旧手写校验等价（强度只增不减）；导出供测试复用。
 */
export const dreDecisionSchema = z.object({
  action: z.enum(["observe", "reflect", "act"]),
  content: z.string(),
  confidence: z.number().min(0).max(1),
});

/** 意识流决策（zod 推导，与 isDreDecision 守卫一致） */
export type DreDecision = z.infer<typeof dreDecisionSchema>;

/** 校验 LLM 输出是否满足决策 schema（确定性，无 LLM） */
export function isDreDecision(value: unknown): value is DreDecision {
  return dreDecisionSchema.safeParse(value).success;
}
