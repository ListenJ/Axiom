...
class ContextEngineImpl {
  ...
  build(input: string, history: Array<{ role: string; content: string }> = []): RuntimeContext {
    ...
    // Get goals
    const goals = Array.from(worldState.query("goals.").entries()).map(([key, value]) => ({
      id: key,
    ...
    // Get beliefs
    const beliefs = Array.from(worldState.query("beliefs.").entries()).map(([key, value]) => ({
      statement: (value as any)?.statement ?? key,
      confidence: (value as any)?.confidence ?? 0.5,
    }));
