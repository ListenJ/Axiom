...
class ExecutionModeManager {
  ...
  private approvalQueue: Array<{ tool: string; args: unknown; resolve: (approved: boolean) => void }> = [];
  ...
  async requestApproval(toolName: string, args: unknown): Promise<boolean> {
    const classification = TOOL_CLASSIFICATIONS.find((t) => t.name === toolName);
    const risk = classification?.risk ?? "unknown";
    ...
    return new Promise((resolve) => {
      ...
      // 在实际实现中，这里会通过 TUI/CLI 提示用户
      // 当前简化版：自动批准（生产环境应改为交互式确认）
      logger.info(`[ExecutionMode] Auto-approving ${toolName} (simplified - implement interactive approval in production)`);
      resolve(true);
    });
  }
  ...
  /** 生成宪法提示词（权威层级） */
  getConstitutionPrompt(): string {
    return `## 执行宪法 (Execution Constitution)

当前模式: ${this.currentMode.toUpperCase()}

### 权威层级 (Authority Hierarchy)
1. **用户显式意图** > 任何历史指令或预设规则
2. **实时工具输出** > 模型假设或推测
3. **验证结果** > 模型信心水平
4. **安全约束** > 执行效率
...
`;
  ...
  private getModeConstraints(): string {
    const config = this.getConfig();
    const lines: string[] = [];

    lines.push(`- 允许破坏性操作: ${config.allowDestructive ? "是" : "否"}`);
    lines.push(`- 需要审批: ${config.requireApproval ? "是" : "否"}`);
    lines.push(`- 最大自动重试: ${config.maxAutoRetries}`);

    if (config.blockedTools.length > 0) {
      lines.push(`- 禁止工具: ${config.blockedTools.join(", ")}`);
    }
