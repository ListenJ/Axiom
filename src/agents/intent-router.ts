...
/** 大类意图映射规则 */
const CATEGORY_INTENTS: Record<string, { keywords: string[]; label: string; defaultAgent?: string; recommendedRole?: TaskRole }> = {
  engineering: {
...
/** 将识别到的意图映射为任务角色（用于智能任务分配） */
export function mapIntentToRole(intentResult: IntentResult | null): TaskRole {
  if (!intentResult) return "coding";

  const role = CATEGORY_INTENTS[intentResult.intent]?.recommendedRole;
  if (role) return role;

  // 基于意图的默认回退映射
  const fallbackMap: Record<string, TaskRole> = {
    engineering: "main_coding",
    design: "research",
    product: "research",
    marketing: "research",
    sales: "research",
    ...
    "game-development": "main_coding",
    strategy: "research",
    "spatial-computing": "main_coding",
    specialized: "research",
    integrations: "coding",
    "paid-media": "research",
  };

  return fallbackMap[intentResult.intent] || "coding";
}
