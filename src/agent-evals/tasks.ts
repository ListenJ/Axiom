...
const knowledge: AgentTask[] = [
  ...
  t("KNOW-02", "knowledge", "train", "Bun 与 Node 差异",
    ...
    (r) => containsAllAny(r, [["zig"], ["jsc", "javascriptcore", "v8", "引擎", "engine", "运行时", "性能"], ["typescript"]]),
    { maxTokens: 512,
  ...
  t("KNOW-05", "knowledge", "train", "容器 vs 虚拟机",
    ...
    (r) => containsAllAny(r, [["共享内核", "宿主机内核"], ["隔离", "namespace", "cgroup"], ["启动", "秒级", "毫秒", "引导"]]),
    { maxTokens: 512,
      expectedBehavior: "说明容器共享宿主机内核、以 namespace/cgroup 隔离、启动快等与虚拟机的区别" }),
...
const selfEvolve: AgentTask[] = [
  ...
  t("EVOLVE-06", "self-evolve", "held-out", "工具误用自检清单",
    ...
    (r) => containsAllAny(r, [["路径", "path"], ["确认", "检查", "验证"]]),
    { maxTokens: 256,
      expectedBehavior: "给出 rm -rf 前 3 条自检项，覆盖路径确认与检查验证等安全手段" }),
