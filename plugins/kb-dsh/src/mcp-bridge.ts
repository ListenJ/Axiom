/**
 * axiom-kb-dsh MCP 桥 —— 以 stdio 拉起插件内置知识库后端，把「KB 白名单」内的工具
 * 以 `kb__<tool>` 名称注册进 dsh ToolRuntime，并把模型可见输出渲染为纯文本。
 * 实现参照 dsh 官方 mcp-client（packages/mcp/mcp-client）与 plugins/dsh 的桥：
 * 用 uncached tools/list + tools/call，避免 SDK 输出校验器缓存。
 *
 * 与 plugins/dsh 单块桥的唯一差异：按 toolFilter 白名单过滤（默认 KB 族：
 * Vault 记忆 + 知识图谱；联网检索工具不在此列）。
 */
...
/**
 * 生成模型可见的公开工具名：`<serverName>__<rawName>`（本插件 serverName='kb'）。
...
/** KB 能力面默认白名单：前缀（以 _ 结尾）或全名。 */
export const DEFAULT_KB_FILTER: string[] = [
  'memory_',
  'code_index',
  'kg_',
  'kal_',
...
export function extractText(content: unknown, rawName: string): string {
  if (!Array.isArray(content)) return `[kb:${rawName}] no text content`
  const parts: string[] = []
...
export function toToolDefinition(tool: McpToolMeta, opts: McpBridgeOptions, getClient: () => Client | null): DshToolDefinition {
  ...
  return {
    ...
    execute: async (args, exec) => {
      ...
      if (!client) throw new Error(`[axiom-kb-dsh] MCP client not connected for tool ${rawName}`)
      ...
      if (result.isError === true) {
        throw new Error(`[kb:${rawName}] ${extractText(result.content, rawName)}`)
      }
...
export function createMcpBridge(opts: McpBridgeOptions): McpBridge {
  ...
  // 子进程环境 = 继承当前进程环境 + 显式覆盖（opts.env 优先，便于注入 OBSIDIAN_VAULT_PATH 等）
  ...
  async function connect(ctx: DshContext): Promise<void> {
    ...
    const client = new Client({ name: 'axiom-kb-dsh', version: '0.1.0' }, { capabilities: {} })
    ...
    // 白名单过滤：只注册知识库能力面工具（Vault 记忆 + 知识图谱），
    // 联网检索等其它工具面不会进入 dsh
    ...
    ctx.logger?.info?.(
      `[axiom-kb-dsh] bridged ${selected.length}/${tools.length} KB tools from built-in MCP server (${opts.serverName})`,
    )
  }
