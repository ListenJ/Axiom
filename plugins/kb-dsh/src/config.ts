/**
 * axiom-kb-dsh 配置解析 —— 纯函数，便于单元测试。
 *
 * 所有可调项都有默认值；dsh 侧可经 cordis.patch.yml 或 $DSH_HOME/cordis.patch.yml
 * 按行 id `kb` 覆盖整段 config。插件始终使用内置后端（Vault 记忆 + 知识图谱，
 * backend/server.js，Bun 单文件构建），数据目录默认 <插件>/data 并自动创建。
 */
import path from 'node:path'
import { fileURLToPath } from 'node:url'

export interface AxiomKbConfig {
  /** 后端数据目录（Vault 笔记 + 知识图谱 SQLite）；默认 <插件>/data，自动创建。 */
  dataDir: string
  /** 是否拉起内置知识库后端（stdio）并桥接 KB 白名单工具。 */
  mcpEnabled: boolean
  mcpCommand: string
  mcpArgs: string[]
  mcpEnv: Record<string, string>
  /** MCP 工具公开名前缀（kb__<tool>）。 */
  mcpServerName: string
  mcpToolCallTimeoutMs: number
  /** 初始连接/工具同步失败时是否让插件 fiber 失败（默认容忍并记录）。 */
  mcpFailOnStartupError: boolean
  /**
   * 工具白名单：前缀（以 `_` 结尾，如 `memory_`）或全名（如 `code_index`）。
   * 空数组 = 使用内置 KB 白名单（DEFAULT_KB_FILTER）。
   */
  toolFilter: string[]
}

export type NormalizedConfig = AxiomKbConfig
...
/** 解析插件根目录：源码布局 plugins/kb-dsh/src、产物布局 plugins/kb-dsh/lib → 上溯 1 层。 */
