...
- **宽度**：`w-[min(25rem,62vw)]`；内层卡片 `m-2 rounded-2xl`（四周 8px 留白 + 16px 圆角）——视觉上为**悬浮圆角玻璃卡片**，非贴边面板。
- **材质**：`.overlay-glass`（暗 rgba(22,22,22,.16) / 亮 rgba(255,255,255,.18)，`blur(36px) saturate(1.5)`）——透明度略高于背景毛玻璃，透出背景光效，靠高斯模糊保证可读性；**无丝绸衬底**；`panel-shadow-left` 左侧深投影（-14px 0 48px -20px）分隔（无边框）。
