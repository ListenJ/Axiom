...
- **不占空间**：桌面右栏为 `absolute -top-4 -bottom-4 right-2 z-30` 的**悬浮浮层**（负偏移突破主内容 padding），不参与布局、不推挤工作区（聊天内容保持全宽）；**贴齐工作区上下边界**（实测 top 56 / bottom 892，工作区 48–900 仅各留 8px）；默认收起、按需唤起（打开时浮层盖过工具栏右侧）。
- **宽度**：`w-[min(25rem,62vw)]`，16px 圆角——**悬浮圆角玻璃卡片**。
- **材质**：`.overlay-glass`（暗 rgba(22,22,22,.30) / 亮 rgba(255,255,255,.30)，**`blur(56px) saturate(1.6)` 高磨砂**）——低透光 + 高磨砂，玻璃感与可读性兼得；**无丝绸衬底**；深投影分隔（无边框）。
...
- `.overlay-glass`：**`blur(56px) saturate(1.6)` 高磨砂**，背景暗 rgba(22,22,22,.30) / 亮 rgba(255,255,255,.30)（低透光、文字更清晰，仍透出背景光效）；移动抽屉保留 `box-shadow: 0 28px 80px rgba(0,0,0,.78)` + 顶部柔光。
...
- 材质探针：blur(56px)、无 `::before` 丝绸衬底（暗/亮）；Git 面板边框元素 0；`/` 15/15 完整、150 次 API 无 429。
