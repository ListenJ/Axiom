...
describe("GracefulDegradationRouter", () => {
  it("should initialize with default config", () => {
    const gd = new GracefulDegradationRouter();
    expect(gd).toBeDefined();
  });

  it("should get degradation path for role", () => {
    const gd = new GracefulDegradationRouter();
    const path = gd.getDegradationPath("general-chat");
    expect(path).toBeDefined();
    expect(path.primary).toBeDefined();
    expect(path.fallbacks).toBeDefined();
    expect(Array.isArray(path.fallbacks)).toBe(true);
  });

  it("should get degradation path preferring free models", () => {
    const gd = new GracefulDegradationRouter();
    const path = gd.getDegradationPath("general-chat", { preferFree: true });
    expect(path).toBeDefined();
    expect(path.primary).toBeDefined();
  });

  it("should aggregate results with best strategy", () => {
    const gd = new GracefulDegradationRouter();
    const results = [
      {
        role: "general-chat" as const,
        model: "model-a",
        provider: "provider-a",
        endpoint: "",
        content: "Result A",
    ...
    const aggregated = gd.aggregateResults(results, "best");
  ...
  it("should aggregate results with merge strategy", () => {
    ...
    const aggregated = gd.aggregateResults(results, "merge");
    expect(aggregated).toBeDefined();
    expect(aggregated).toContain("--- Result 1 ---");
    expect(aggregated).toContain("--- Result 2 ---");
  });

  it("should aggregate results with vote strategy", () => {
    const gd = new GracefulDegradationRouter();
    const results = [
      {
        role: "general-chat" as const,
    ...
    const aggregated = gd.aggregateResults(results, "vote");
    expect(aggregated).toBeDefined();
    expect(typeof aggregated).toBe("string");
  });

  it("should aggregate results with fastest strategy", () => {
    const gd = new GracefulDegradationRouter();
    const results = [
      {
        role: "general-chat" as const,
    ...
    const aggregated = gd.aggregateResults(results, "fastest");
    expect(aggregated).toBeDefined();
  });

  it("should get health report", () => {
    const gd = new GracefulDegradationRouter();
    const report = gd.getHealthReport();
...
describe("Integration: Context Takeover", () => {
  ...
  it("should handle graceful degradation initialization", () => {
    const gd = new GracefulDegradationRouter();
    expect(gd).toBeDefined();
    const path = gd.getDegradationPath("decision");
    expect(path.parallelEnabled).toBeDefined();
    expect(typeof path.parallelEnabled).toBe("boolean");
  });
});