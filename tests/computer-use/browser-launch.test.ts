...
describe("resolveOpenCommand", () => {
  it("Windows 用 cmd /c start", () => {
    expect(resolveOpenCommand("https://example.com", "win32")).toEqual(["cmd", "/c", "start", "", "https://example.com"]);
  });
  it("Linux 用 xdg-open", () => {
  ...
  it("空 URL 抛错", () => {
    expect(() => resolveOpenCommand("  ", "win32")).toThrow(/non-empty url/);
  });
});
